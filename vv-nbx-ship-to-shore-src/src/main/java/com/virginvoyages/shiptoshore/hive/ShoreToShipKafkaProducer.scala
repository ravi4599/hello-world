package com.virginvoyages.shiptoshore.hive
...
object ShoreToShipKafkaProducer {
  ...
  def getKafkaProducer(sparkConf: Broadcast[Map[String,String]]) = {
   ...
   def getKafkaProducerNonSSL(sparkConf: Broadcast[Map[String,String]]) = {
    val props = new Properties()
    props.put(ProducerConfig.BOOTSTRAP_SERVERS_CONFIG, sparkConf.value.get("spark.target.nonsslkafkabrokers").get.trim())
    props.put(ProducerConfig.KEY_SERIALIZER_CLASS_CONFIG, classOf[StringSerializer])
    props.put(ProducerConfig.VALUE_SERIALIZER_CLASS_CONFIG, classOf[StringSerializer])
