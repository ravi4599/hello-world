...
/*************************************calling metadata framework*********************************************/
      ...
      import org.apache.spark.sql.functions.{ unix_timestamp, to_date }
       val selectQuery = """(SELECT DISTINCT p1.PERSON_FIRST_NAME as FirstName, p1.PERSON_LAST_NAME as LastName, p1.PERSON_FIRST_NAME_PREFERRED as Name, lower(pa.PERSON_E_MAIL) as Email, p1.PERSON_ID as UserID, p1.PERSON_FIRST_STAY as HireDate, po.POSITION_NAME as JobTitle, dep.ORG_DEPT_NAME as Department, null as JobFamily,	p1.PERSON_AVAILABILITY_UNTIL_DATE as TerminationDate, GETDATE() as "Export Date",	null as TrackID, null as IsManager,	null as State, null as Country FROM dbo.Persons p1 JOIN ( SELECT max(pb2.ARRIVAL_DATE) as LatestArrivalDate, pb2.PERSON_ID FROM dbo.Person_Booking pb2 WHERE pb2.MANIFEST_TYPE = 'E' and pb2.PERSON_BOOKING_STATUS_ID = 1 and pb2.REC_DELETED = 0 GROUP BY pb2.PERSON_ID) pbsub ON p1.PERSON_ID = pbsub.PERSON_ID JOIN dbo.Person_Booking pb ON pbsub.PERSON_ID = pb.PERSON_ID and pbsub.LatestArrivalDate = pb.ARRIVAL_DATE JOIN dbo.Person_Address pa ON p1.PERSON_ID = pa.PERSON_ID JOIN dbo.Positions po ON pb.POSITION_ID = po.POSITION_ID JOIN dbo.Org_Depts dep ON po.DEPARTMENT_ID = dep.ORG_DEPT_NAME_ID WHERE 1=1 and p1.active = 1 and p1.PERSON_TYPE_ID = 2 and p1.PERSON_FIRST_STAY <= GETDATE() and (p1.PERSON_AVAILABILITY_DATE >= GETDATE() or p1.PERSON_AVAILABILITY_DATE is null) and pa.PERSON_IS_MAILING_ADDRESS = 1 and pa.PERSON_ADDRESS_TYPE_ID = 1 ORDER BY p1.PERSON_LAST_NAME, p1.PERSON_FIRST_NAME OFFSET 0 ROWS) as betterup"""
      ...
      /*  val data = spark.read.format("jdbc").options(Map("url" ->":5432/SEAWARE", "user" -> "rep_user", "password" ->"reportdev@321",
		...
		/*	val now = Calendar.getInstance.getTime
		 ...
		 /* val nowZoned = ZonedDateTime.now();
			val midnight = nowZoned.toLocalDate().atStartOfDay(nowZoned.getZone()).toInstant();
			val duration = Duration.between(midnight, Instant.now());
			val seconds = duration.getSeconds(); 
			val seq=seconds.toString() */
			
			// val fileName=spark.sqlContext.sparkContext.getConf.get("spark.target.filename").concat("_").concat(seq).concat("_").concat(DateTimeFormatter.ofPattern("MMddyyyy").format(LocalDateTime.now))
		...
		//  Backup target: s3://vv-dev-emr-cluster/data/landing/bkupBetterup/
