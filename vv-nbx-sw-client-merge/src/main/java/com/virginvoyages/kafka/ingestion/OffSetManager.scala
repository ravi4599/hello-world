...
object OffSetManager {
  ...
  def saveOffsets(
    ...
    try {
       ...
       val op_mode = configMap.value.get("spark.metadata.offset.mode").get
      ...
      case e: SQLException => { log.info("Postgre db connectioin issue..please check Postgre db service"); throw new HBaseConnetionException("Postgre db Connection Exception") }
  ...
  def getLastCommittedOffset(TOPIC: String, sqlContext: SQLContext, spark: SparkSession): Map[TopicPartition, Long] =
    {
      ...
      try {
        println(s"[START]----- reading postgre table jdbc record as dataframe")
        ...
        */
        ...
        case e: SQLException => {
          e.printStackTrace()
          throw new HBaseConnetionException("Postgre db Connection Exception")
        }
        case e: Exception => {
          e.printStackTrace()
