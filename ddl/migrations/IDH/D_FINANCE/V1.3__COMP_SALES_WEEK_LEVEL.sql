CREATE OR REPLACE VIEW COMP_SALES_WEEK_LEVEL (
    BRAND_ID,
    OWNERSHIP_TYPE,
    OWNER_ID,
    REST_ID,
...
max_owner_by_store_in_week as (
    ...
    from
        eligibility_week_with_date elig
        LEFT JOIN IDM_%%envname%%.COREDIM_BV.RESTAURANT_SCD_DIM_BV rest ON rest.store_id = elig.rest_id
        and rest.brand_id = elig.brand_id
        and greatest(rest.effective_begin_dt, elig.WK_START_DATE) >= elig.WK_START_DATE
        and LEAST(rest.effective_end_dt, elig.WK_END_DATE) <= elig.WK_END_DATE QUALIFY newest_dup <= 1
),
daypart_sum as (
    select
        dp.brand_id,
...
preagg as (
    ...
    from
        ...
        LEFT JOIN daypart_sum ON daypart_sum.brand_id = eligibility_week_with_date.brand_id
        AND daypart_sum.rest_id = eligibility_week_with_date.rest_id
        AND daypart_sum.yr_nbr = eligibility_week_with_date.yr_nbr
        AND daypart_sum.wk_nbr = eligibility_week_with_date.wk_nbr
        LEFT JOIN daypart_sum last ON last.brand_id = eligibility_week_with_date.brand_id
        AND last.rest_id = eligibility_week_with_date.rest_id
        AND last.yr_nbr = eligibility_week_with_date.last_yr
        AND last.wk_nbr = eligibility_week_with_date.wk_nbr
        LEFT JOIN daypart_sum prev ON prev.brand_id = eligibility_week_with_date.brand_id
        AND prev.rest_id = eligibility_week_with_date.rest_id
        AND prev.yr_nbr = eligibility_week_with_date.prev_yr
        AND prev.wk_nbr = eligibility_week_with_date.wk_nbr
        LEFT JOIN daypart_sum prev_prev ON prev_prev.brand_id = eligibility_week_with_date.brand_id
        AND prev_prev.rest_id = eligibility_week_with_date.rest_id
        AND prev_prev.yr_nbr = eligibility_week_with_date.prev_prev_yr
        AND prev_prev.wk_nbr = eligibility_week_with_date.wk_nbr
)
select
    BRAND_ID,
    CASE
