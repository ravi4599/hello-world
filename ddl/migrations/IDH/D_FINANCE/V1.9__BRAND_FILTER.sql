create or replace view BOS_DAILY_SALES_MEASURE(
	BUSINESS_DATE,
	BRAND_ID,
	REST_ID,
	MEASURE_NAME,
...
INNER JOIN IDS_DEV.TXN_BV.FN_SYSTEM_BV s
 ...
 on 1 = 1
INNER JOIN (select FN_MEASURE_ID, BRAND_ID from IDS_DEV.TXN.FN_BRAND_MEASURE where SALES_RECON_REPORT_DISPLAY_IND='TRUE' and BRAND_ID= 'arbys') FBM on m.fn_measure_id = FBM.FN_MEASURE_ID and r.brand_id = FBM.BRAND_ID
...
INNER JOIN (select FN_MEASURE_ID, BRAND_ID from IDS_DEV.TXN.FN_BRAND_MEASURE where SALES_RECON_REPORT_DISPLAY_IND='TRUE' and BRAND_ID= 'bww') FBM on m.fn_measure_id = FBM.FN_MEASURE_ID and r.brand_id = FBM.BRAND_ID
  LEFT OUTER JOIN   (
...
INNER JOIN  IDS_DEV.TXN_BV.FN_MEASURE_BV m on 1 = 1
INNER JOIN (select FN_MEASURE_ID, BRAND_ID from IDS_DEV.TXN.FN_BRAND_MEASURE where SALES_RECON_REPORT_DISPLAY_IND='TRUE' and BRAND_ID= 'sonic') FBM on m.fn_measure_id = FBM.FN_MEASURE_ID and r.brand_id = FBM.BRAND_ID
  LEFT OUTER JOIN   (
    SELECT dsd.* FROM IDS_DEV.TXN_BV.FN_DAILY_REV_MEASURE_BV dsd
   INNER JOIN IDS_DEV.TXN_BV.FN_SYSTEM_BV s
  on s.FN_SYSTEM_CATEGORY_CODE = 'BOS' 
...
create or replace view BOS_WEEKLY_SALES_MEASURE(
    FISC_WK_END_DATE,
    BRAND_ID,
    REST_ID,
    MEASURE_NAME,
...
create or replace view GL_WEEKLY_SALES_MEASURE(
    FISC_WK_END_DATE,
    BRAND_ID,
    REST_ID,
    MEASURE_NAME,
...
create or replace view POS_DAILY_SALES_MEASURE(
    BUSINESS_DATE,
    BRAND_ID,
    REST_ID,
    MEASURE_NAME,
...
create or replace view POS_WEEKLY_SALES_MEASURE(
    FISC_WK_END_DATE,
    BRAND_ID,
    REST_ID,
    MEASURE_NAME,
