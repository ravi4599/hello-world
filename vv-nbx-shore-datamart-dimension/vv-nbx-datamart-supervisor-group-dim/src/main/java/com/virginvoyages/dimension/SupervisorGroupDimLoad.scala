...
object SupervisorGroupDimLoad {
  ...
  def main(args: Array[String]) {
    ...
    try {
      val whereClause = s""" where item.batchtime>= '$batch_start_tme' and item.batchtime<='$batch_end_tme' and item.part_date>='$part_read_start' and item.part_date<='$part_read_end'"""
