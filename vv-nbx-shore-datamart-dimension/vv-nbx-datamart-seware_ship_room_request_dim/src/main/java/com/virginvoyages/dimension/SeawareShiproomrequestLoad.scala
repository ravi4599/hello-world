...
object SeawareShiproomrequestLoad {
  ...
  def main(args: Array[String]): Unit = {
    ...
    try {
      ...
      val ActSlotDF = spark.sql(query).as("activity-slot-dim")
      println(query)
      log.info(query)
      
     
   
      if (!ActSlotDF.head(1).isEmpty) {
        loadDimFact(spark, ActSlotDF)
      ...
      case e: Exception =>
        { ManageMetadata.updateStatus(batch_instance_id1, batch_id1, "Failed", spark); log.info("******************in the vv-nbx-datamart-activity-slot-dims ******************"); e.printStackTrace(); throw new Exception("General Exception..please check the stacktrace", e); }
