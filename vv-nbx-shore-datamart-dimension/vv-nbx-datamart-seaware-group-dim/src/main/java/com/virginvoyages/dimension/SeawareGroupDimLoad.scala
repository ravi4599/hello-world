...
/*************************************calling metadata framework*********************************************/
    ...
    try {
     ...
     val whereClause = s""" where gb.batchtime>= '$batch_start_tme' and gb.batchtime<='$batch_end_tme' and gb.part_date>='$part_read_start' and gb.part_date<='$part_read_end'"""
      println(s"""#---------------------------Starting the Execution--for whereClause ----------------#""")
      log.info(s"""#---------------------------Starting the Execution--for whereClause ----------------#""")

      
      val stageGroupDimDF =  spark.sql(spark.sparkContext.getConf.get("spark.source.sql").trim().replace("*whereclause*", whereClause) )
      /*.replace("*whereclause*", whereClause)*/
          /*.trim()+whereClause)*/
     /* val stageVoyageDF = spark.sql(spark.sparkContext.getConf.get("spark.source.sql"))*/
      stageGroupDimDF.show(4,false)
      
      //val interDF=stageGroupDimDF.select($"src_group_id",$"group_init_date",$"group_status",$"group_type",$"group_name",$"group_mode",$"is_internal",$"source_code",$"office_code",$"operator",$"n_of_guests",$"cancellation_date",greatest($"ts_ms_gb", $"ts_ms_gh") as "src_date")

val interDF=stageGroupDimDF.select($"src_group_id",$"group_init_date",$"group_status",$"group_type",$"group_name",$"group_mode",$"is_internal",$"source_code",$"office_code",$"operator",$"n_of_guests",$"cancellation_date",$"comments",$"agency_id",$"agent_id",$"sail_date_from",$"sail_date_to",$"ship_id",$"is_active",$"is_valid",$"currency_code",$"sec_agency_id",$"is_charter",$"currency_rate",$"first_full_payment_processed",$"sec_agent_id",$"client_id",$"package_id",greatest($"ts_ms_gb", $"ts_ms_gh") as "src_date")
