...
/*************************************calling metadata framework*********************************************/
    ...
    try {
     ...
     val whereClause = s""" where batchtime>= '$batch_start_tme' and batchtime<='$batch_end_tme' and part_date>='$part_read_start' and part_date<='$part_read_end'"""
      ...
      val TransactionEvtDF =  spark.sql(spark.sparkContext.getConf.get("spark.source.sql").trim().replace("*whereclause*", whereClause))
          /*.trim()+whereClause)*/
     /* val stageVoyageDF = spark.sql(spark.sparkContext.getConf.get("spark.source.sql"))*/
      ...
      for (col <- required_columns) {
        ...
        if (hasColumn(TransactionEvtDF, col)) {
      ...
      val stage_final_df = missing_columns.foldLeft(TransactionEvtDF)((df, c) =>
     ...
     if (!stage_final_df.head(1).isEmpty) {
      ...
      case e: SQLException => { ManageMetadata.updateStatus(batch_instance_id1, batch_id1, "Failed", spark); log.info("******************in the catch of transaction evt  Load ******************"); e.printStackTrace(); throw new Exception("SQL Exception..please check the stacktrace", e); }

      case e: Exception =>
       { ManageMetadata.updateStatus(batch_instance_id1, batch_id1, "Failed", spark); log.info("******************in the catch of transaction evt   Load ******************"); e.printStackTrace(); throw new Exception("General Exception..please check the stacktrace", e); }
