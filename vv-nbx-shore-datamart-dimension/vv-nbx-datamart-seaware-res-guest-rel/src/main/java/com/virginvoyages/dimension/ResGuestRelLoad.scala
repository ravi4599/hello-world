...
/*************************************calling metadata framework*********************************************/
    ...
    try {
     ...
     val whereClause = s""" where batchtime>= '$batch_start_tme' and batchtime<='$batch_end_tme' and part_date>='$part_read_start' and part_date<='$part_read_end'"""
      ...
      val stageResGuestRelDF =  spark.sql(spark.sparkContext.getConf.get("spark.source.sql").trim().replace("*whereclause*", whereClause))
          /*.trim()+whereClause)*/
     /* val stageVoyageDF = spark.sql(spark.sparkContext.getConf.get("spark.source.sql"))*/
     // stageResGuestRelDF.show(4,false)
      ...
      for (col <- required_columns) {
        ...
        if (hasColumn(stageResGuestRelDF, col)) {
      ...
      val stage_final_df = missing_columns.foldLeft(stageResGuestRelDF)((df, c) =>
    ...
    //  stage_final_df.show(5,false)
     ...
     if (!stage_final_df.head(1).isEmpty) {
      //loadDimFact(spark: SparkSession, stage_final_df)
       ...
       var tempDf = stage_final_df.withColumn("rowId", monotonically_increasing_id())
       tempDf = tempDf.withColumn(spark.sparkContext.getConf.get("spark.surrogate_key.column"), row_number().over(w) + 0).drop("rowId").withColumn("etl_ld_status",lit("No change")).withColumn("load_dt", current_timestamp).withColumn("upd_dt", current_timestamp)
       tempDf.createOrReplaceTempView("temp")
       val final_df= spark.sql("select cast(res_guest_id as bigint) as res_guest_id,res_id,guest_id,guest_seqn,rec_start_dttm,rec_end_dttm,etl_ld_status,load_dt,upd_dt from temp")
