...
/*************************************calling metadata framework*********************************************/
   ...
   try{
    val whereClause = s""" where al.BatchTime >= '$batch_start_tme' and al.BatchTime<='$batch_end_tme' and al.Part_date>='$part_read_start' and al.Part_date<='$part_read_end'"""
      ...
      var SeawareFlightDf = spark.sql(spark.sparkContext.getConf.get("spark.source.sql").trim().replace("*whereclause*", whereClause))
      
      
 
      
      if(!SeawareFlightDf.head(1).isEmpty){
        
      log.info("calling scd framework")
      loadDimFact(spark: SparkSession, SeawareFlightDf)
      ...
      ManageMetadata.updateStatus(batch_instance_id1, batch_id1, "Successful", spark)
       ...
       case e: SQLException => { 
         ...
         log.info("******************in the catch of Seaware Flight Dim Load ******************");
         e.printStackTrace(); 
