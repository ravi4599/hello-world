...
/*************************************calling metadata framework*********************************************/
    ...
    try {
     ...
     /* val stageVoyageDF = spark.sql(spark.sparkContext.getConf.get("spark.source.sql"))*/
      ...
      val interDF=stageSailDimDF.select($"src_sail_id",$"sail_days",$"sail_route_code",$"sail_port_from",$"sail_port_to",$"sail_geog_area_code",$"dep_ref_id",$"arr_ref_id",$"sail_date_from",$"sail_date_to",$"is_active",$"ship_id",$"is_fake",greatest($"ts_ms_sh", $"ts_ms_sa_act",$"ts_ms_sa_act1") as "src_date")
      ...
      println("xxxxxxxxxxxxxxxxxxxxxxxxxxxxx-------------------------------Source-Stage Sail------------------------------------xxxxxxxxxxxxxxxxxxxxxx")
     ...
     if (!stage_final_df.head(1).isEmpty) {
      ...
      case e: SQLException => { ManageMetadata.updateStatus(batch_instance_id1, batch_id1, "Failed", spark); log.info("******************in the catch of Sail Dimension Load ******************"); e.printStackTrace(); throw new Exception("SQL Exception..please check the stacktrace", e); }

      case e: Exception =>
       { ManageMetadata.updateStatus(batch_instance_id1, batch_id1, "Failed", spark); log.info("******************in the catch of Sail Dimension Load ******************"); e.printStackTrace(); throw new Exception("General Exception..please check the stacktrace", e); }
