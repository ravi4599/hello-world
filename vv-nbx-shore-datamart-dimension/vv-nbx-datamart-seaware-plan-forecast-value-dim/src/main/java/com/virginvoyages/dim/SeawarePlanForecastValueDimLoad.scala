...
object SeawarePlanForecastValueDimLoad {
...
/*************************************calling metadata framework*********************************************/
   ...
   try{
    //val whereClause = s""" where al.BatchTime >= '$batch_start_tme' and al.BatchTime<='$batch_end_tme' and al.Part_date>='$part_read_start' and al.Part_date<='$part_read_end'"""
     // println(s"""#---------------------------Starting the Execution--for $whereClause ----------------#""")
      //log.info(s"""#---------------------------Starting the Execution--for $whereClause ----------------#""")
      
      import spark.sqlContext.implicits._
      var SeawarePlanForecastValueDf = spark.sql(spark.sparkContext.getConf.get("spark.source.sql").trim())
          //.replace("*whereclause*", whereClause)
      
      
 
      
      if(!SeawarePlanForecastValueDf.head(1).isEmpty){
        
      log.info("calling scd framework")
      loadDimFact(spark: SparkSession, SeawarePlanForecastValueDf)
      ...
      ManageMetadata.updateStatus(batch_instance_id1, batch_id1, "Successful", spark)
       ...
       case e: SQLException => { 
         ...
         log.info("******************in the catch of Seaware Plan Forecast Value Dim Load ******************");
         e.printStackTrace(); 
