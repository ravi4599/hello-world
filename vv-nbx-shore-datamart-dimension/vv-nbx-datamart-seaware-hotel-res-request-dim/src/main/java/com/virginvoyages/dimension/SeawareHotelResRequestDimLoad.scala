...
object SeawareHotelResRequestDimLoad {
...
/*************************************calling metadata framework*********************************************/
   ...
   try{
    val whereClause = s""" where r.BatchTime >= '$batch_start_tme' and r.BatchTime<='$batch_end_tme' and r.Part_date>='$part_read_start' and r.Part_date<='$part_read_end'"""
      ...
      var SeawareHotelResRequestDf = spark.sql(spark.sparkContext.getConf.get("spark.source.sql").trim().replace("*whereclause*", whereClause))
      
      
 
      
      if(!SeawareHotelResRequestDf.head(1).isEmpty){
        
      log.info("calling scd framework")
      loadDimFact(spark: SparkSession, SeawareHotelResRequestDf)
      ...
      ManageMetadata.updateStatus(batch_instance_id1, batch_id1, "Successful", spark)
       ...
       case e: SQLException => { 
         ...
         log.info("******************in the catch of Seaware Hotel Res Request Dim Load ******************");
         e.printStackTrace(); 
