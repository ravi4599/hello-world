...
object CrmConversationDefinitionSessionLoad {
...
var DeviceDf = spark.sql(spark.sparkContext.getConf.get("spark.source.sql").trim())
//.replace("*whereclause*", whereClause))


if(!DeviceDf.head(1).isEmpty){

log.info("Loading ConversationDefinitionSession Dim Table")
