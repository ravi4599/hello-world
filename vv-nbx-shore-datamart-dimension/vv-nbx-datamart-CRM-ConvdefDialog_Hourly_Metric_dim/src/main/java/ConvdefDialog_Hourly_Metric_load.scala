...
object ConvdefDialog_Hourly_Metric_load {
...
/*************************************calling metadata framework*********************************************/
   ...
   val whereClause = s""" where batchtime>= '$batch_start_tme' and batchtime<='$batch_end_tme' and part_date>='$part_read_start' and part_date<='$part_read_end'"""
      ...
      var ActivityDF = spark.sql(spark.sparkContext.getConf.get("spark.source.sql").trim()+whereClause)
    ...
    val missing_columns = ArrayBuffer[String]()
     ...
     print(missing_columns, "Here are the missing columns")
      val stage_final_df = missing_columns.foldLeft(ActivityDF)((df, c) =>
      ...
      ManageMetadata.updateStatus(batch_instance_id1, batch_id1, "Successful", spark)
       ...
       case e: SQLException => {
         ...
         log.info("******************in the catch of Activity Dimension Load ******************");
         e.printStackTrace();
