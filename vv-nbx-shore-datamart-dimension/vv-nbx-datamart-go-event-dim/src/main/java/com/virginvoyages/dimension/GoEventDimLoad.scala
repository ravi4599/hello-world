...
import org.apache.spark.sql.types.{StructType, StructField, TimestampType,StringType, IntegerType,DateType,LongType,BooleanType};
...
/*************************************calling metadata framework*********************************************/
    ...
    try {
      ...
      //var event_dim = spark.sql(spark.sparkContext.getConf.get("spark.source.sql").trim() + whereClause )
      //event_dim.withColumn("event_deleted_flg", col("event_deleted_flg").cast(BooleanType))
	  var event_dim = spark.sql(spark.sparkContext.getConf.get("spark.source.sql").trim().replace("*whereclause*", whereClause ))
      event_dim.withColumn("event_deleted_flg", col("event_deleted_flg").cast(BooleanType))
      ...
      val stage_final_df = missing_columns.foldLeft(event_dim)((df, c) =>
