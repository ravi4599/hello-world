...
/*    if (args.length <= 1) {
    ...
    try {
      ...
      var cticallskilltabledf = spark.sql("select * from %s %s".format(sc.getConf.get("spark.target.ctiskill.table"), whereClause))

       if (!cticallskilltabledf.head(1).isEmpty) {
 /*     val ctiteamdimdf = spark.sql(spark.sparkContext.getConf.get("spark.teamdim.sql").trim())
         
      cticallskilldatadf = cticallskilldatadf.join(ctiteamdimdf, cticallskilldatadf("teamId") === ctiteamdimdf("cti_team_dim_teamId"), "left")
      
     //cticallskilldatadf.show()
      
      var cticallskilltabledf = cticallskilldatadf.select("agentId","cti_team_dim_cti_team_skey","userName","firstName","lastName","emailAddress","isActive","isSupervisor","profileId","profileName","location","etl_load_dt")
      
      cticallskilltabledf = cticallskilltabledf.withColumnRenamed("cti_team_dim_cti_team_skey", "teamnewId")
     //cticallskilltabledf.show(10,false)
      //cticallskilltabledf.printSchema()
      
     cticallskilltabledf = cticallskilltabledf.withColumn("teamnewId", when(cticallskilltabledf.col("teamnewId").isNull, lit(-1)).otherwise(cticallskilltabledf.col("teamnewId")))
*/
      //cticallskilltabledf.show(10,false)
     // cticallskilltabledf.printSchema()
