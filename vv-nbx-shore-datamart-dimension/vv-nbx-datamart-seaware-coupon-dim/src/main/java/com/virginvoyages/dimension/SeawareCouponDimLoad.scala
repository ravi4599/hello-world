...
/*************************************calling metadata framework*********************************************/
    ...
    try {
     ...
     val whereClause = s""" where ac.batchtime>= '$batch_start_tme' and ac.batchtime<='$batch_end_tme' and ac.part_date>='$part_read_start' and ac.part_date<='$part_read_end'"""
      println(s"""#---------------------------Starting the Execution--for whereClause ----------------#""")
      log.info(s"""#---------------------------Starting the Execution--for whereClause ----------------#""")

      
      val stageCouponDimDF =  spark.sql(spark.sparkContext.getConf.get("spark.source.sql").trim().replace("*whereclause*", whereClause))
      /*.replace("*whereclause*", whereClause)*/
          /*.trim()+whereClause)*/
     /* val stageVoyageDF = spark.sql(spark.sparkContext.getConf.get("spark.source.sql"))*/
      //stageCouponDimDF.show(4,false)
      
      val interDF=stageCouponDimDF.select($"src_coupon_id",$"user_defined_coupon_id",$"issue_date",$"valid_from",$"valid_to",$"effective_from",$"effective_to",$"coupon_category",$"coupon_class",$"coupon_class_code",$"coupon_class_comments",$"apply_as_payment",$"apply_as_discount",$"office_code",$"res_id",$"entity_type",$"entity_id",$"is_used",$"is_active",$"currency_code",$"amount",$"amount_left",$"charge_code",$"reason_code",$"comments",greatest($"ts_ms_ac", $"ts_ms_cc",$"ts_ms_va") as "src_date")
      ...
      println("xxxxxxxxxxxxxxxxxxxxxxxxxxxxx-------------------------------Source-Stage Coupon------------------------------------xxxxxxxxxxxxxxxxxxxxxx")
