...
/*************************************calling metadata framework*********************************************/
    ...
    try {
      ...
      val boardingnumberdf = spark.sql(spark.sparkContext.getConf.get("spark.source.sql").trim()+whereClause) 
      //.where($"batchtime" >= lit(batch_start_tme).cast(TimestampType) && $"batchtime" < lit(batch_end_tme).cast(TimestampType) && $"part_date" >= lit(part_read_start).cast(DateType) && $"part_date" < lit(part_read_end).cast(DateType))
	  val boardingnumdatetime = boardingnumberdf.withColumn("boarding_slot_start_datetime",concat(split(col("effectivefromdate"), " ")(0), lit(" "), col("boardingslotstarttime")).cast(TimestampType)).withColumn("boarding_slot_end_datetime",concat(split(col("effectivetodate"), " ")(0), lit(" "), col("boardingslotendtime")).cast(TimestampType)).drop(col("effectivefromdate")).drop(col("effectivetodate")).drop(col("boardingslotstarttime")).drop(col("boardingslotendtime"))
    boardingnumdatetime.show()  
      ...
      for (col <- required_columns) {
        ...
        if (hasColumn(boardingnumdatetime, col)) {
      ...
      val stage_final_df = missing_columns.foldLeft(boardingnumdatetime)((df, c) =>
        df.withColumn(s"$c", lit(null)))

      println("xxxxxxxxxxxxxxxxxxxxxxxxxxxxx-------------------------------Source-Stage boarding number------------------------------------xxxxxxxxxxxxxxxxxxxxxx")
      stage_final_df.show()
      ...
      case e: SQLException => { ManageMetadata.updateStatus(batch_instance_id1, batch_id1, "Failed", spark); log.info("******************in the catch of boarding number Dimension Load ******************"); e.printStackTrace(); throw new Exception("SQL Exception..please check the stacktrace", e); }

      case e: Exception =>
        { ManageMetadata.updateStatus(batch_instance_id1, batch_id1, "Failed", spark); log.info("******************in the catch of boarding number Dimension Load ******************"); e.printStackTrace(); throw new Exception("General Exception..please check the stacktrace", e); }
