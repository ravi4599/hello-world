...
/*************************************calling metadata framework*********************************************/
    ...
    try {
     ...
     val whereClause = s""" where batchtime>= '$batch_start_tme' and batchtime<='$batch_end_tme' and part_date>='$part_read_start' and part_date<='$part_read_end'"""
      ...
      val stageSeawareGuestVxpRelDF =  spark.sql(spark.sparkContext.getConf.get("spark.source.sql").trim().replace("*whereclause*", whereClause))
          /*.trim()+whereClause)*/
     ...
     // stageResGuestRelDF.show(4,false)
      ...
      for (col <- required_columns) {
        ...
        if (hasColumn(stageSeawareGuestVxpRelDF, col)) {
      ...
      val stage_final_df = missing_columns.foldLeft(stageSeawareGuestVxpRelDF)((df, c) =>
