...
object SeawareBookedCabinReservation {
...
/*************************************calling metadata framework*********************************************/
    ...
    try {
     ...
     //val whereClause = s""" where gb.batchtime>= '$batch_start_tme' and gb.batchtime<='$batch_end_tme' and gb.part_date>='$part_read_start' and gb.part_date<='$part_read_end'"""
      ...
      val stageBookedCabinReservationDimDF =  spark.sql(spark.sparkContext.getConf.get("spark.source.sql").trim().replace("*whereclause*", whereClause) )
      /*.replace("*whereclause*", whereClause)*/
          /*.trim()+whereClause)*/
     /* val stageVoyageDF = spark.sql(spark.sparkContext.getConf.get("spark.source.sql"))*/
      stageBookedCabinReservationDimDF.show(4,false)
      
      println(stageBookedCabinReservationDimDF.count)
      val interDF=stageBookedCabinReservationDimDF.select($"src_res_id",$"src_guest_id",$"res_id", $"guest_id", $"package_id", $"price_category", $"price_category_rank", $"berth_category", $"berth_category_rank", $"assigned_category", $"assigned_category_rank", $"generic_category", $"allocation_id", $"cabin_id", $"sail_cabin_reserve_id",greatest($"ts_ms_sr", $"ts_ms_ia") as "src_date")
     ...
     if (!stage_final_df.head(1).isEmpty) {
       ...
       //stage_final_df.where($"res_id"===3183018).show(false)
