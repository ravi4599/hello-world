...
object PosSaleDetailDimLoad {
 ...
 /* var stagesale_detailDF = spark.sql(spark.sparkContext.getConf.get("spark.source.sql").trim())//.where($"batchtime" >= lit(batch_start_tme).cast(TimestampType) && $"batchtime" <= lit(batch_end_tme).cast(TimestampType) && $"part_date" >= lit(part_read_start).cast(DateType) && $"part_date" <= lit(part_read_end).cast(DateType))
  stagesale_detailDF=stagesale_detailDF.where($"batchtime" >= lit(batch_start_tme).cast(TimestampType) && $"batchtime" <= lit(batch_end_tme).cast(TimestampType) && $"part_date" >= lit(part_read_start).cast(DateType) && $"part_date" <= lit(part_read_end).cast(DateType))
  //stagesale_detailDF.show(20,false)*/
    val stagesale_detailDF = spark.sql(spark.sparkContext.getConf.get("spark.source.sql").trim()).where($"batchtime" >= lit(batch_start_tme).cast(TimestampType) && $"batchtime" <= lit(batch_end_tme).cast(TimestampType) && $"part_date" >= lit(part_read_start).cast(DateType) && $"part_date" <= lit(part_read_end).cast(DateType))
  ...
  for ( col <-required_columns ) 
        { 
            ...
            if(hasColumn(stagesale_detailDF, col) ){
  ...
  val stage_final_df=missing_columns.foldLeft(stagesale_detailDF)((df, c) =>
  df.withColumn(s"$c",  lit(null))
)
//stage_final_df.show()


  println("xxxxxxxxxxxxxxxxxxxxxxxxxxxxx-------------------------------Source-Stage sale_detail------------------------------------xxxxxxxxxxxxxxxxxxxxxx")
  
  
  //stage_final_df.show(20,false)
