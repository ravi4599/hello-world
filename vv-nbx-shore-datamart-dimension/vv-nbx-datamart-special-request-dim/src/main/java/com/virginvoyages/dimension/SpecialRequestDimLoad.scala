...
/*************************************calling metadata framework*********************************************/
    ...
    try {
      ...
      val stageSpecialRequestDF = spark.sql(spark.sparkContext.getConf.get("spark.source.sql").trim().replace("*whereclause*", whereClause))
      ...
      for (col <- required_columns) {
        ...
        if (hasColumn(stageSpecialRequestDF, col)) {
      ...
      val stage_final_df = missing_columns.foldLeft(stageSpecialRequestDF)((df, c) =>
        df.withColumn(s"$c", lit(null)))

      println("xxxxxxxxxxxxxxxxxxxxxxxxxxxxx-------------------------------Source-Stage SpecialRequest------------------------------------xxxxxxxxxxxxxxxxxxxxxx")
      //stage_final_df.show()
      ...
      case e: SQLException => { ManageMetadata.updateStatus(batch_instance_id1, batch_id1, "Failed", spark); log.info("******************in the catch of SpecialRequest Dimension Load ******************"); e.printStackTrace(); throw new Exception("SQL Exception..please check the stacktrace", e); }

      case e: Exception =>
        { ManageMetadata.updateStatus(batch_instance_id1, batch_id1, "Failed", spark); log.info("******************in the catch of SpecialRequest Dimension Load ******************"); e.printStackTrace(); throw new Exception("General Exception..please check the stacktrace", e); }
