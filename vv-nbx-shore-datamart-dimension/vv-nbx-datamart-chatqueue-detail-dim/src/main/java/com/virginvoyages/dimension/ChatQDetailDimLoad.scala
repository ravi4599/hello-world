...
val log = LogManager.getRootLogger
		...
		val spark = getSparkSession()
				...
				try{
					...
					//  val positionDF = spark.sql(spark.sparkContext.getConf.get("spark.source.sql").trim()).where($"batchtime" >= lit(batch_start_tme).cast(TimestampType) && $"batchtime" <= lit(batch_end_tme).cast(TimestampType) && $"part_date" >= lit(part_read_start).cast(DateType) && $"part_date" <= lit(part_read_end).cast(DateType))
