...
/*************************************calling metadata framework*********************************************/
    ...
    try {
       ...
       val whereClause = s""" where batchtime>= '$batch_start_tme' and batchtime<='$batch_end_tme' and partdate>='$part_read_start' and partdate<='$part_read_end'"""
      ...
      var address_Dim = spark.sql(spark.sparkContext.getConf.get("spark.source.sql").trim()+ whereClause )
      ...
      val stage_final_df = missing_columns.foldLeft(address_Dim)((df, c) =>
