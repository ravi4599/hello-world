 ...
 */
case class HeirarchyRel(sparkSession: SparkSession) extends Serializable {

  private[this] def deepTopLevelHierarcy(vertexNodeDF: DataFrame, edgeRelationShipDF: DataFrame):
    RDD[(Any, (Int, Any, String, Int, Int))] = {
    vertexNodeDF.show(false)
    edgeRelationShipDF.show(false)
    val vRDD = vertexNodeDF
      .rdd
      .map{x => (x.get(0), x.get(1), x.get(0))}
  ...
  def compute(empStructDF: DataFrame): DataFrame = {
    ...
    val (empVertexNodeDF, empEdgeRelationShipDF) = (
      empStructDF.selectExpr("id", "parentroleid","portalrole"),
      empStructDF.selectExpr("parentroleid", "id").filter($"parentroleid".isNotNull))
    ...
    val empHirearchyDeepNodeDF = deepTopLevelHierarcy(empVertexNodeDF, empEdgeRelationShipDF)
      ...
      .toDF("parentroleid2", "level", "root", "path", "iscyclic", "isleaf").persist()

    empHirearchyDeepNodeDF.join(empStructDF,
      empStructDF.col("id") === empHirearchyDeepNodeDF.col("parentroleid2"))
      .selectExpr("id","portalrole", "parentroleid", "level", "root", "path", "iscyclic", "isleaf")
  }
}
