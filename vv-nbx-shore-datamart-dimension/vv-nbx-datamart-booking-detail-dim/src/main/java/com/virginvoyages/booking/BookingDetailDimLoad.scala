...
/*************************************calling metadata framework*********************************************/
   ...
   try{
    val whereClause = s""" where bkng.batchtime>= '$batch_start_tme' and bkng.batchtime<='$batch_end_tme' and bkng.Part_Date>='$part_read_start' and bkng.Part_Date<='$part_read_end'"""
      ...
      var bookingDf = spark.sql(spark.sparkContext.getConf.get("spark.source.sql").trim()+whereClause)
      
      
      
   if(!bookingDf.head(1).isEmpty){
        
      log.info("calling scd framework")
      loadDimFact(spark: SparkSession, bookingDf)
      ...
      ManageMetadata.updateStatus(batch_instance_id1, batch_id1, "Successful", spark)
       ...
       case e: SQLException => { 
         ...
         log.info("******************in the catch of Booking Detail Load ******************");
         e.printStackTrace(); 
