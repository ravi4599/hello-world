...
/*************************************calling metadata framework*********************************************/
    ...
    try {
      ...
      println("#---------------------------Starting the Execution------------------#")
       val whereClause = s""" and batchtime>= '$batch_start_tme' and batchtime<='$batch_end_tme' and part_date>='$part_read_start' and part_date<='$part_read_end'"""

      var stagesale_detailDF =  spark.sql(spark.sparkContext.getConf.get("spark.source.sql").trim()+ whereClause )
     
      stagesale_detailDF=stagesale_detailDF.withColumn("sale_detail_src", lit("FOLIO"))
      ...
      case e: SQLException => {
        ...
        log.info("******************in the catch of sale_detail folio Dimension Load ******************");
      ...
      case e: Exception =>
        {
          ManageMetadata.updateStatus(batch_instance_id1, batch_id1, "Failed", spark);
          log.info("******************in the catch of sale_detail folio Dimension Load ******************"); e.printStackTrace(); throw new Exception("General Exception..please check the stacktrace", e);
        }
