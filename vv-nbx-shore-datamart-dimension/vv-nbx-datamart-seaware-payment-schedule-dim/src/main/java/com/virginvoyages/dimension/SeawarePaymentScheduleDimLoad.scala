...
object SeawarePaymentScheduleDimLoad {
...
/*************************************calling metadata framework*********************************************/
   ...
   try{
    val whereClause = s""" where pm.BatchTime >= '$batch_start_tme' and pm.BatchTime<='$batch_end_tme' and pm.Part_date>='$part_read_start' and pm.Part_date<='$part_read_end'"""
      println(s"""#---------------------------Starting the Execution--for $whereClause ----------------#""")
      log.info(s"""#---------------------------Starting the Execution--for $whereClause ----------------#""")
      
      import spark.sqlContext.implicits._
      var SeawarePaymentScheduleDf = spark.sql(spark.sparkContext.getConf.get("spark.source.sql").trim().replace("*whereclause*", whereClause))
      
      
 
      
      if(!SeawarePaymentScheduleDf.head(1).isEmpty){
        
      log.info("calling scd framework")
      loadDimFact(spark: SparkSession, SeawarePaymentScheduleDf)
      ...
      ManageMetadata.updateStatus(batch_instance_id1, batch_id1, "Successful", spark)
       ...
       case e: SQLException => { 
         ...
         log.info("******************in the catch of Seaware Payment Schedule Dim Load ******************");
         e.printStackTrace(); 
