package com.virginvoyages.dimensions
...
object PersonTrainingDimLoad {
  	val log = LogManager.getRootLogger
		...
		val spark = getSparkSession()
				...
				try{
					...
					val whereClause = s""" where batchtime>= '$batch_start_tme' and batchtime<='$batch_end_tme' and part_date >='$part_read_start' and part_date <='$part_read_end'"""
							...
							println(s"==========================whereClause : $whereClause =========================")
							log.info (s"=====================whereClause : $whereClause =============================")

							

							var personTrainingDf = spark.sql(spark.sparkContext.getConf.get("spark.source.sql").trim().replace("*whereclause*", whereClause))
							
							personTrainingDf.printSchema()
							personTrainingDf.show(5,false)
							

							if(!personTrainingDf.head(1).isEmpty){

								log.info("Loading PersonTraining Dim table")
								loadDimFact(spark: SparkSession, personTrainingDf)
		...
		case e: SQLException => { 
			...
			log.info("******************in the catch of PersonTraining Dim Load ******************");
			e.printStackTrace(); 
