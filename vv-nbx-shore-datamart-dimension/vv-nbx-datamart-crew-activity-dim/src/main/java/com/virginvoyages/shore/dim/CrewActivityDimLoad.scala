...
/*************************************calling metadata framework*********************************************/
   ...
   try{
    val whereClause = s""" where BatchTime >= '$batch_start_tme' and BatchTime<='$batch_end_tme' and Part_date>='$part_read_start' and Part_date<='$part_read_end'"""
      ...
      var crewActivityDf = spark.sql(spark.sparkContext.getConf.get("spark.source.sql").trim().replace("*whereclause*", whereClause))
      
      
 
      
      if(!crewActivityDf.head(1).isEmpty){
        
      log.info("calling scd framework")
      loadDimFact(spark: SparkSession, crewActivityDf)
      ...
      ManageMetadata.updateStatus(batch_instance_id1, batch_id1, "Successful", spark)
       ...
       case e: SQLException => { 
         ...
         log.info("******************in the catch of Crew Activity Dim Load ******************");
         e.printStackTrace(); 
