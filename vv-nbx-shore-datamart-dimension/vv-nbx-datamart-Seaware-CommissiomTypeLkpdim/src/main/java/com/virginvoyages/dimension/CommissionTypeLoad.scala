...
/*************************************calling metadata framework*********************************************/
    ...
    try {
     ...
     //val whereClause = s""" where batchtime>= '$batch_start_tme' and batchtime<='$batch_end_tme' and part_date>='$part_read_start' and part_date<='$part_read_end'"""
      ...
      val stageCommissionTypeDF =  spark.sql(spark.sparkContext.getConf.get("spark.source.sql").trim())
      ...
      for (col <- required_columns) {
        ...
        if (hasColumn(stageCommissionTypeDF, col)) {
      ...
      val stage_final_df = missing_columns.foldLeft(stageCommissionTypeDF)((df, c) =>
