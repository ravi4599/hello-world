...
object SeawareTransferRequestLkp {
...
/*************************************calling metadata framework*********************************************/
    ...
    try {
     ...
     /*val whereClause = s""" where poin.batchtime>= '$batch_start_tme' and poin.batchtime<='$batch_end_tme' and poin.part_date>='$part_read_start' and poin.part_date<='$part_read_end'"""
      println(s"""#---------------------------Starting the Execution--for whereClause ----------------#""")
      log.info(s"""#---------------------------Starting the Execution--for whereClause ----------------#""")*/

      
      val stageVoyageDF =  spark.sql(spark.sparkContext.getConf.get("spark.source.sql").trim())
          /*.trim()+whereClause)*/
     /* val stageVoyageDF = spark.sql(spark.sparkContext.getConf.get("spark.source.sql"))*/
      stageVoyageDF.show(4,false)
      ...
      for (col <- required_columns) {
        ...
        if (hasColumn(stageVoyageDF, col)) {
      ...
      val stage_final_df = missing_columns.foldLeft(stageVoyageDF)((df, c) =>
