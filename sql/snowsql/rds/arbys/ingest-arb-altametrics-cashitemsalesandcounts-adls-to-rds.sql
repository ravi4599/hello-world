...
COPY INTO RDS_{{params.env}}.{{params.schema}}.CASH_ITEM_SALES_AND_COUNTS
FROM 	(
  SELECT
	$1:: STRING AS  StoreID,
    ...
    LEFT(RIGHT(METADATA$FILENAME,55),8) AS FOLDERDATE
    FROM @RDS_{{params.env}}.{{params.schema}}.ODI_READ_CSV_FROM_ADLS_TWO/{{params.run_date}}/ 
    (file_format => '{{params.file_format}}', pattern => '{{params.ciscpattern}}')
);
