...
WITH FILTERED_REC AS (
  ...
  FROM (
    ...
    WHERE BRAND_CD = 'BR' AND SALES_REPORT_FREQ = 'Monthly'
  )
  WHERE ROW_NUM = 1
)
SELECT 
'bskn' AS BRAND_ID,
to_char(LAST_DAY(BUSINESS_DATE),'YYYY-MM-DD')  REPORTING_PERIOD,
...
TO_DATE(DATE_TRUNC('MONTH',ADD_MONTHS('{{ var.json.baskin_polled_sales_monthly.load_dt | default((macros.datetime.today() ).strftime('%Y-%m-%d') ) }}',-1))) AND LAST_DAY(ADD_MONTHS('{{ var.json.baskin_polled_sales_monthly.load_dt | default((macros.datetime.today() ).strftime('%Y-%m-%d') ) }}',-1))
--AND DAY(TO_DATE('{{ var.json.baskin_polled_sales_monthly.load_dt | default((macros.datetime.today() ).strftime('%Y-%m-%d') ) }}')) < 11
GROUP BY 
  T.BRAND_ID,
...
COPY INTO @IDH_{{target_env}}.D_FINANCE.STAGE_FOR_BASKIN_POLLED_SALES/BRI_UDP_Finance_PolledSales_{{ var.json.baskin_polled_sales_monthly.load_dt | default((macros.datetime.today() ).strftime('%Y%m%d') ) }}_MONTHLY.dat
FROM (
SELECT 'BRI' BRAND, to_char(CAST(REPORTING_PERIOD AS date),'MM-DD-YYYY')  REPORTING_PERIOD, CUSTOMER_NUMBER, CUSTOMER_NAME, rest_id as UNIT, LOCAL_CURRENCY, LOCAL_NET_SALES_RETAIL, TRANSACTION_COUNT_RETAIL, LOCAL_GROSS_SALES, LOCAL_NET_SALES_WHOLESALE, TRANSACTION_COUNT_WHOLESALE, REPORTING_FREQUENCY FROM IDS_{{target_env}}.FINANCE_ACCTG.FRANCHISEE_POLLED_SALES_SNAPSHOT  WHERE SNAPSHOT_DATE = current_date AND BRAND_ID = 'bskn' AND REPORTING_FREQUENCY = 'MONTHLY')
OVERWRITE = TRUE 
single = true
