...
AND BRAND_ID='sonic' and FN_SYSTEM_ID=7 ;


CREATE OR REPLACE TEMPORARY TABLE "IDS_{{params.target_env}}".TXN.FN_DAILY_REV_MEASURE_SDI_BOS_INFOR_TEMP AS
...
FROM
(
  WITH FILTERED_DATES_INFOR_SALES_RECORDS AS
  (
        SELECT Distinct To_date(SUBSTR(t1.raw_data, 5, 8),'MMDDYYYY') as Business_Date 
        FROM "RDS_{{params.source_env}}"."SDI_BV"."BOS_INFOR_SALES_BV" t1
  ...
  FILTERED_INFOR_SALES_RECORDS AS
  (
    SELECT
    ...
    FROM
      ...
      CASE
       ...
       WHEN SUBSTR(t1.raw_data, 23,5) = '11432' THEN 'DISCOVER' --Calculating Discover
       WHEN SUBSTR(t1.raw_data, 23,5) = '50915' AND t1.raw_data LIKE '%NTDISCOUNT%' THEN 'NON TAXABLE DISCOUNT' --Calculating Discounts
       WHEN SUBSTR(t1.raw_data, 23,5) = '50915'  AND t1.raw_data LIKE '%TEDISCOUNT%' THEN 'TAX EXEMPT DISCOUNT'
       WHEN SUBSTR(t1.raw_data, 23,5) = '50915'  THEN 'DISCOUNT'
      ...
      THEN
       ...
       END
       ELSE to_double(concat(LEFT(SUBSTR(t1.raw_data, 34,13), LEN(SUBSTR(t1.raw_data, 34,13))-2),'.',
                                   SUBSTR(SUBSTR(t1.raw_data, 34,13),-2)))
      END AS amount,
      RANK() OVER (PARTITION BY SUBSTR(t1.filename, 33,17) 
                            ORDER BY t1.folderdate DESC, t1.filedate DESC, SUBSTR(t1.filename, 51,6) DESC, t1.loaddatetime DESC) AS rank       
      FROM "RDS_{{params.source_env}}"."SDI_BV"."BOS_INFOR_SALES_BV" t1
      INNER JOIN "IDS_{{params.source_env}}".LOCN_BV.FN_REST_ICR_OPENCLOSE_DATE lo on 
      lo.brand_id = 'sonic' AND
      LPAD(SUBSTR(t1.raw_data, 1,4),5 ,0) = lo.REST_ID 
      ...
      Where To_date(SUBSTR(t1.raw_data, 5, 8),'MMDDYYYY') in (Select Business_Date from FILTERED_DATES_INFOR_SALES_RECORDS)                         
      )
  WHERE rank = 1
  ),
  FILTERED_INFOR_SALES_RECORDS_FINAL AS
  (
    SELECT
    storeid, salesdate,rn,description,
    CASE WHEN UPPER(description) IN ('TAX EXEMPT','NON TAXABLE','TAXES') AND R.RN = 2 THEN 'FOOD SALES' 
    	 WHEN UPPER(description) IN ('TAX EXEMPT DISCOUNT') AND R.RN = 2 THEN 'TAX EXEMPT'
    	 WHEN UPPER(description) IN ('NON TAXABLE DISCOUNT') AND R.RN = 2 THEN 'NON TAXABLE'
    	  WHEN UPPER(description) IN ('TAX EXEMPT DISCOUNT') AND R.RN = 1 THEN 'DISCOUNT'
    	 WHEN UPPER(description) IN ('NON TAXABLE DISCOUNT') AND R.RN = 1 THEN 'DISCOUNT'
         WHEN UPPER(description) IN ('PAIDIN', 'PAIDOUT') THEN 'PAID OUT'    ELSE description END AS Measure_Desc, --Calculating Netfood sales need to do duplicate some measure. Formula: (FOOD SALES + TAX EXEMPT - TAXES - DISCOUNT).
         --Calculating TAX EXEMPT and NON TAXABLE sales need to do duplicate some measure. Formula: (TE Sale - TE Discount, NTSale - NT Discount).
    account_number,
    CASE WHEN (UPPER(description) IN ('TAXES','TAX EXEMPT DISCOUNT','NON TAXABLE DISCOUNT') AND R.RN = 2) OR UPPER(description) ='PAIDIN' THEN -1 * amount
    ELSE amount END AS amount
    FROM FILTERED_INFOR_SALES_RECORDS
    INNER JOIN (SELECT 1 AS RN UNION ALL SELECT 2 AS RN) R
    ON (UPPER(description) IN ('TAX EXEMPT','NON TAXABLE','TAXES','TAX EXEMPT DISCOUNT','NON TAXABLE DISCOUNT') AND R.RN = 2) OR R.RN = 1
  )
  SELECT
  'sonic'                                                      AS brand_id,
  ...
  FROM FILTERED_INFOR_SALES_RECORDS_FINAL cisc  
  ...
  AND fsmr.fn_system_id = '7' AND fsmr.brand_id='sonic'
...
SELECT
	...
	sale_amount, country_code, currency_code, source_system_name, load_id, load_dttm, update_id, update_dttm
FROM  "IDS_{{params.target_env}}".TXN.FN_DAILY_REV_MEASURE_SDI_BOS_INFOR_TEMP
) ams
ON   
