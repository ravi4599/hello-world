...
SET start_wk_dt = (SELECT REPLACE((fiscal_week_end_dt -7),'-','') AS start_wk_dt
                           FROM "IDS_{{params.source_env}}"."INT_REF_BV"."DATE_DIM_BV" dv  WHERE dv.date_key = $start_load_dt);
SET end_wk_dt = (SELECT REPLACE((fiscal_week_end_dt),'-','')
					       FROM "IDS_{{params.source_env}}"."INT_REF_BV"."DATE_DIM_BV" dv WHERE dv.date_key = $end_load_dt);
...
(
  ...
  (
    ...
    FROM "IDS_{{params.source_env}}"."TXN"."FN_DAILY_REV_MEASURE" ss
    JOIN "IDS_{{params.source_env}}"."INT_REF_BV"."DATE_DIM_BV" dv
	ON ss.business_date = dv.calendar_dt WHERE ss.fn_system_id IN ('7', '8')
	and ss.brand_id ='sonic' AND ss.fn_measure_id IN (17)
  ),
  ...
  FROM
	(
	 SELECT
	 fdss.brand_id, fdss.rest_id, fdss.fn_system_id, fdss.fn_measure_id, fdss.gl_account_code, fdss.gl_cost_ctr,
	 fdss.sale_usd_amount, fdss.sale_count, fdss.country_code, fdss.currency_code, fdss.source_system_name,
	 fdss.business_date,
	 ...
	 FROM "IDS_{{params.source_env}}"."TXN"."FN_DAILY_REV_MEASURE" fdss
	 JOIN "IDS_{{params.source_env}}"."INT_REF_BV"."DATE_DIM_BV" dv ON fdss.business_date = dv.calendar_dt
     WHERE fn_system_id IN ('7', '8') AND fdss.brand_id = 'sonic' AND fn_measure_id IN (17)
     AND dv.fiscal_week_end_dt BETWEEN (SELECT COALESCE(sdate, udate) FROM DATE_TO_BE_PROCESSED)
  ...
  (
   ...
   CASE WHEN FDS.fn_system_id='7' THEN  '5' 
        WHEN FDS.fn_system_id='8' THEN '6' 
        ELSE '0' END AS fn_system_id,
   fds.fn_measure_id AS fn_measure_id, fds.gl_account_code AS gl_account_code, fds.gl_cost_ctr AS gl_cost_ctr,
   ...
   INNER JOIN "IDS_{{params.source_env}}"."INT_REF_BV"."DATE_DIM_BV" dv
   ON fds.business_date = dv.calendar_dt
   GROUP BY fds.brand_id, dv.fiscal_week_end_dt, fds.rest_id, fds.fn_system_id, fds.fn_measure_id, fds.gl_account_code, 
            fds.gl_cost_ctr, fds.country_code, fds.currency_code, fds.source_system_name
  )
  SELECT
