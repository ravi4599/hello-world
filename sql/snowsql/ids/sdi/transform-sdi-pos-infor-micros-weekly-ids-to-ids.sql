...
AND BRAND_ID='sonic' and FN_SYSTEM_ID in (5,6);
...
(
  ...
  (
    ...
    WHERE ss.fn_system_id IN ('5','6') AND ss.brand_id = 'sonic'
  ),
  ...
  (
   ...
   (
    ...
    WHERE fn_system_id IN ('5','6') AND dv.fiscal_week_end_dt  BETWEEN
   ...
   WHERE row_num = 1 AND brand_id = 'sonic'
  ),
  MASTER_TXNS AS
  ( 
   SELECT
