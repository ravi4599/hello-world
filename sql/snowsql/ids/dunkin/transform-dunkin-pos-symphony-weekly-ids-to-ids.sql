...
--SET start_load_dt = '20220701';
--SET end_load_dt = '20230415';                      
...
SET start_wk_dt = (SELECT REPLACE((fiscal_week_end_dt -7),'-','') AS start_wk_dt
                   FROM "IDS_{{params.target_env}}"."INT_REF_BV"."DATE_DIM_BV" dv  
				   WHERE dv.date_key = $start_load_dt);
SET end_wk_dt = (SELECT REPLACE((fiscal_week_end_dt),'-','')
				 FROM "IDS_{{params.target_env}}"."INT_REF_BV"."DATE_DIM_BV" dv 
				 WHERE dv.date_key = $end_load_dt);
...
(
  ...
  WHERE ss.fn_system_id = '17' AND ss.brand_id ='dnkn'
  ),
  ...
  SELECT 
    ...
    (
      ...
      fdss.gl_cost_ctr, fdss.sale_usd_amount, fdss.sale_count, fdss.country_code, fdss.currency_code,
      fdss.source_system_name,fdss.business_date,
      ...
      WHERE fn_system_id = '17' AND fdss.brand_id = 'dnkn' AND dv.fiscal_week_end_dt BETWEEN
  ...
  sale_usd_amount, sale_amount, country_code, currency_code, source_system_name,
...
WHEN MATCHED THEN
UPDATE SET
  fwrm.sale_usd_amount = pbs.sale_usd_amount, fwrm.sale_amount = pbs.sale_amount,
  fwrm.country_code = pbs.country_code, fwrm.currency_code = pbs.currency_code,
  fwrm.source_system_name = pbs.source_system_name, fwrm.update_id = pbs.update_id, fwrm.update_dttm = pbs.update_dttm
WHEN NOT MATCHED
THEN INSERT
(
