...
/*************************************calling metadata framework*********************************************/
      ...
      var maxID = spark.sql(spark.sparkContext.getConf.get("spark.hive.table.maxid")).first().get(0)
...
//      val selectQueryTmp = "select * from " + temptab
      ...
      val whereQueryClause = """ where """ + conditionColumn + """ > """ + maxID1   
      ...
      val data = spark.read.format(sparkConfiguration.value.get("spark.src.con.format").get).options(Map("url" -> sparkConfiguration.value.get("spark.src.con.url").get, "user" -> sparkConfiguration.value.get("spark.src.user").get, "password" -> sparkConfiguration.value.get("spark.src.password").get,
        "dbtable" -> SelectQuerywithWhere, "driver" -> sparkConfiguration.value.get("spark.src.con.driver").get)).load() 
      data.show(false)
      ...
      if (!filteredDataDF.head(1).isEmpty) {
        ...
        val source_column_list2 = filteredDataDF.columns.toSeq
        val source_column_count2 = source_column_list2.length
      ...
      //  val finaleQuery = "select " + spark.sparkContext.getConf.get("spark.source.columns") + s""",VoyageId,BatchTime,ShipCode,Part_Date from $temp_tab"""
        println(finaleQuery)
        val stageFinalDf = spark.sql(finaleQuery)
        stageFinalDf.printSchema
        stageFinalDf.show(false)

        val source_column_list1 = stageFinalDf.columns.toSeq
        val source_column_count1 = source_column_list1.length
