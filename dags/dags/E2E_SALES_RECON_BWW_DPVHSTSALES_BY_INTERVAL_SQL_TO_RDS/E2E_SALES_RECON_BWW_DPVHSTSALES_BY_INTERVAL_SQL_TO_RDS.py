...
af_params = Variable.get('E2E_SALES_RECON_BWW_DPVHSTSALES_BY_INTERVAL_SQL_TO_RDS', default_load_variable, deserialize_json=True)
...
#Future date comparing 
today = pendulum.now('America/New_York').add(days=0).format('YYYYMMDD')
yesterday=pendulum.now('America/New_York').add(days=-1).format('YYYYMMDD')

today_dt = datetime.strptime(today, "%Y%m%d")
yesterday_dt = datetime.strptime(yesterday, "%Y%m%d")


today_date = datetime.strftime(today_dt, "%Y-%m-%d")
yesterday_date = datetime.strftime(yesterday_dt, "%Y-%m-%d")

if ld_end_date > today_date:
       ld_end_date=yesterday_date
       print('Future Date:',yesterday_date,'today:',today_date) 
    
else :print(ld_end_date,'historical date ') 
...
with DAG(
    ...
    E2E_SALES_RECON_BWW_DPVHSTSALES_BY_INTERVAL_SQL_TO_ADLS = RunADFPipelineoperator(
       task_id='E2E_SALES_RECON_BWW_DPVHSTSALES_BY_INTERVAL_SQL_TO_ADLS',
    ...
    E2E_SR_SALESBYINTERVAL_CHECKFILE_SIZE = RunADFPipelineoperator(
       task_id='E2E_SR_SALESBYINTERVAL_CHECKFILE_SIZE',
    ...
    E2E_SALES_RECON_BWW_DPVHSTSALES_BY_INTERVAL_ADLS_TO_RDS = SnowflakeOperator(
       task_id='E2E_SALES_RECON_BWW_DPVHSTSALES_BY_INTERVAL_ADLS_TO_RDS',
       snowflake_conn_id=snowflake_conn_id,
       sql='rds/bww/ingest-bww-dpvhstsalesinterval-adls-to-rds.sql',
       params={
                "warehouse": warehouse,
                "schema": schema,
                "env": env,
...
# Task Run Flow
Start >> E2E_SALES_RECON_BWW_DPVHSTSALES_BY_INTERVAL_SQL_TO_ADLS
E2E_SALES_RECON_BWW_DPVHSTSALES_BY_INTERVAL_SQL_TO_ADLS >> E2E_SR_SALESBYINTERVAL_CHECKFILE_SIZE
E2E_SR_SALESBYINTERVAL_CHECKFILE_SIZE >> E2E_SALES_RECON_BWW_DPVHSTSALES_BY_INTERVAL_ADLS_TO_RDS
E2E_SALES_RECON_BWW_DPVHSTSALES_BY_INTERVAL_ADLS_TO_RDS >> End
# Task Run Flow End
