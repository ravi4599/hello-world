"""
This DAG loads BWW's ALOHA data from IDS to IDS Weekly table.
"""
...
af_params = Variable.get('E2E_SALES_RECON_BWW_ALOHA_IDS_TO_IDS_WEEKLY', default_load_variable, deserialize_json=True)
...
from_date = af_params['ld_start_dt'] if af_params['ld_start_dt'] != '' \
                                     else pendulum.now('America/New_York').add(days=-90).format('YYYYMMDD') \
                                     if today_dt.weekday() == 6 \
                                     else pendulum.now('America/New_York').add(days=-7).format('YYYYMMDD')
...
#start_date =datetime.strftime(ld_start_dt, "%Y%m%d")
#end_date =datetime.strftime(ld_end_dt, "%Y%m%d")
...
with DAG(
    ...
    tags = ['BWW', 'SALESRECON_BWW', 'IDS_TO_IDS'],
 ...
 E2E_SALES_RECON_BWW_ALOHA_IDS_TO_IDS_WEEKLY = SnowflakeOperator(
    task_id='E2E_SALES_RECON_BWW_ALOHA_IDS_TO_IDS_WEEKLY',
    snowflake_conn_id=snowflake_conn_id,
    sql = 'ids/bww/transform-bww-aloha-weekly-ids-to-ids.sql',
    params={
             "warehouse": warehouse
...
#Task Run Flow
Start>>E2E_SALES_RECON_BWW_ALOHA_IDS_TO_IDS_WEEKLY  
E2E_SALES_RECON_BWW_ALOHA_IDS_TO_IDS_WEEKLY>>End
#Task Run Flow End
