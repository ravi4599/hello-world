...
af_params = Variable.get('E2E_SALES_RECON_ORACLEGL_RDS_TO_IDS', default_load_variable, deserialize_json=True)
...
from_date = af_params['ld_start_dt'] if af_params['ld_start_dt'] != '' \
                                     else pendulum.now('America/New_York').add(days=-180).format('YYYYMMDD') \
...
with DAG(
    ...
    E2E_SALES_RECON_ORACLEGL_RDS_TO_IDS = SnowflakeOperator(
       task_id='E2E_SALES_RECON_ORACLEGL_RDS_TO_IDS',
       snowflake_conn_id=snowflake_conn_id,
       sql='ids/arbys/transform-oraclegl-rds-to-ids.sql',
       params={
                "warehouse": warehouse,
...
# Task Run Flow
Start >> E2E_SALES_RECON_ORACLEGL_RDS_TO_IDS >> End
# Task Run Flow End
