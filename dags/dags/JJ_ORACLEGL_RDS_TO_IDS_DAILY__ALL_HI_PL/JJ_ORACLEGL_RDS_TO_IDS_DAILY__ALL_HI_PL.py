"""
This DAG loads JJ's ORACLEGL data from RDS to IDS Daily table in snowflake.
"""
...
af_params = Variable.get('JJ_ORACLEGL_RDS_TO_IDS_DAILY__ALL_HI_PL', default_load_variable, deserialize_json=True)


today = pendulum.now('US/Eastern').add(days=0).format('YYYYMMDD')
today_dt = datetime.strptime(today, "%Y%m%d")

from_date = af_params['ld_start_dt'] if af_params['ld_start_dt'] != '' else pendulum.now('US/Eastern').add(days=-90).format('YYYYMMDD') if today_dt.weekday() == 6 else pendulum.now('US/Eastern').add(days=-1).format('YYYYMMDD')
to_date = af_params['ld_end_dt'] if af_params['ld_end_dt'] != '' else pendulum.now('US/Eastern').add(days=1).format('YYYYMMDD')
...
with DAG(
    ...
    tags = ['GL', 'SALESRECON_JJ', 'RDS_TO_IDS'],
 ...
 JJ_ORACLEGL_RDS_TO_IDS_DAILY__ALL_HI_PL = SnowflakeOperator(
    task_id='JJ_ORACLEGL_RDS_TO_IDS_DAILY__ALL_HI_PL',
    snowflake_conn_id=snowflake_conn_id,
    sql = 'ids/jj/transform-jj-oraclegl-daily-rds-to-ids.sql',
    params={
             "warehouse": warehouse
...
#Task Run Flow
Start>>JJ_ORACLEGL_RDS_TO_IDS_DAILY__ALL_HI_PL  
JJ_ORACLEGL_RDS_TO_IDS_DAILY__ALL_HI_PL>>End
#Task Run Flow End
