"""
This DAG loads BWW's BOS data from RDS to IDS Daily table in snowflake. -- test
"""
...
af_params = Variable.get('E2E_SALES_RECON_BWW_ALTAMETRICS_RDS_TO_IDS_DAILY', default_load_variable, deserialize_json=True)

today = pendulum.now('America/New_York').add(days=0).format('YYYYMMDD')
today_dt = datetime.strptime(today, "%Y%m%d")

from_date = af_params['ld_start_dt'] if af_params['ld_start_dt'] != '' else pendulum.now('America/New_York').add(days=-90).format('YYYYMMDD') if today_dt.weekday() == 6 else pendulum.now('America/New_York').add(days=-1).format('YYYYMMDD')
...
with DAG(
 ...
 E2E_SALES_RECON_BWW_ALTAMETRICS_RDS_TO_IDS_DAILY = SnowflakeOperator(
    task_id='E2E_SALES_RECON_BWW_ALTAMETRICS_RDS_TO_IDS_DAILY',
    snowflake_conn_id=snowflake_conn_id,
    sql = 'ids/bww/transform-bww-altametrics-daily-rds-to-ids.sql',
    params={
             "warehouse": warehouse
...
#Task Run Flow
Start>>E2E_SALES_RECON_BWW_ALTAMETRICS_RDS_TO_IDS_DAILY>>End
