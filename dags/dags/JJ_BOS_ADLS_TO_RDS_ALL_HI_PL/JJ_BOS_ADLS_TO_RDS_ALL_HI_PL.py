...
af_params = Variable.get('JJ_BOS_ADLS_TO_RDS_ALL_HI_PL', default_load_variable, deserialize_json=True)
...
params_bos_altametrics_sales_raw_table_audit = {
...
dag =  DAG( dag_id = confs.get('dag_id'),
            ...
            tags = ['BOS', 'SALESRECON_JJ', 'ADLS_TO_RDS'],
...
trigger_audit_bos_altametrics = TriggerDagRunOperator(
           task_id='trigger_audit_bos_altametrics',
           trigger_dag_id='UDP_Audit_Validation',
           conf=params_bos_altametrics_sales_raw_table_audit )

start_task.set_downstream(end_task)   
prev_task = start_task
for n in range ((end_date - start_date).days + 1):
        date = start_date + timedelta(n)
        task_id = f'JJ_ALTAMETRICS_{date.strftime("%Y-%m-%d")}'
        date_id = f'{date.strftime("%Y%m%d")}'
        task = SnowflakeOperator(
                        task_id=task_id,
                        snowflake_conn_id=snowflake_conn_id,
                        sql = 'rds/jj/ingest-jj-bos-adls-to-rds.sql',
                        warehouse=warehouse,
                        schema=schema,
                        dag=dag,
                        params={
        ...
        if date == end_date:
            task.set_downstream(trigger_audit_bos_altametrics)
  #End = BashOperator(task_id='End', bash_command='exit 0')
trigger_audit_bos_altametrics.set_downstream(end_task)
