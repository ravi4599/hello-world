...
af_params = Variable.get('ARBYS_POS_ITEM_ADLS_TO_RDS', default_load_variable, deserialize_json=True)
...
with DAG( dag_id = confs.get('dag_id'),
    ...
    prev_task = Start
    with TaskGroup(group_id="ITEM_REVENUE_CATEGORY") as ITEM_REVENUE_CATEGORY:
        for n in range ((end_date - start_date).days + 1):
            date = start_date + timedelta(n)
            task_id = f'ITEM_REVENUE_CATEGORY_DETAILS_{date.strftime("%Y-%m-%d")}'
            date_id = f'{date.strftime("%Y%m%d")}'
            task = SnowflakeOperator(
                        task_id=task_id,
                        snowflake_conn_id=snowflake_conn_id,                        
                        sql = 'rds/arbys/ingest-arb-pos-item-adls-to-rds.sql',
                        warehouse=warehouse,
                        schema=schema,
                        dag=dag,
                        params={
...
Start >> ITEM_REVENUE_CATEGORY >> End
