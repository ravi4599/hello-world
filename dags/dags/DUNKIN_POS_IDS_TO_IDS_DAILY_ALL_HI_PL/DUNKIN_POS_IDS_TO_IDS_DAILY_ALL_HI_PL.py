...
af_params = Variable.get('DUNKIN_POS_IDS_TO_IDS_DAILY_ALL_HI_PL', default_load_variable, deserialize_json=True)

today = pendulum.now('US/Eastern').add(days=0).format('YYYYMMDD')
today_dt = datetime.strptime(today, "%Y%m%d")

from_date = af_params['ld_start_dt'] if af_params['ld_start_dt'] != '' \
                                     else pendulum.now('US/Eastern').add(days=-90).format('YYYYMMDD') \
...
with DAG(
    ...
    tags=['POS', 'SALESRECON_DUNKIN', 'IDS_TO_IDS'],
    ...
    E2E_SALES_RECON_DNKN_IDS_TO_IDS_DAILY = SnowflakeOperator(
       task_id='DUNKIN_POS_IDS_TO_IDS_DAILY_ALL_HI_PL',
       snowflake_conn_id=snowflake_conn_id,
       sql='ids/dunkin/transform-dunkin-pos-symphony-daily-ids-to-ids.sql',
       params={
                "warehouse": warehouse,
...
# Task Run Flow
Start >> E2E_SALES_RECON_DNKN_IDS_TO_IDS_DAILY >> End
# Task Run Flow End
