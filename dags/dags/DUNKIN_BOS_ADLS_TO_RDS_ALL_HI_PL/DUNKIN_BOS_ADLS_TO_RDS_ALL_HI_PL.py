...
af_params = Variable.get('DUNKIN_BOS_ADLS_TO_RDS_ALL_HI_PL', default_load_variable, deserialize_json=True)

file_from_date = af_params['ld_start_dt'] if af_params['ld_start_dt'] != '' \
    else pendulum.now('US/Eastern').add(days=0).format('YYYYMMDD')
...
dag = DAG(dag_id=confs.get('dag_id'),
          ...
          tags=['BOS', 'SALESRECON_DUNKIN', 'ADLS_TO_RDS'],
          dagrun_timeout=timedelta(minutes=600),
          catchup=False
          )

params_bos_crunchtime_sales_audit = {
    ...
    'RDS_TABLE' : 'BOS_CRUNCHTIME_SALES',
    'SOURCE_SYSTEM_NAME' : 'BOS',
    'LOADID_C' : 'LOADID',
    'STAGE_NAME' :  confs.get('stage_name')+'/BOS',
...
trigger_audit_bos_crunchtime_sales = TriggerDagRunOperator(
    task_id='trigger_audit_bos_crunchtime_sales',
    trigger_dag_id='UDP_Audit_Validation',
    conf=params_bos_crunchtime_sales_audit
...
for n in range((end_date - start_date).days + 1):
    ...
    task_id = f'DNKN_CRUNCHTIME_{date.strftime("%Y-%m-%d")}'
    date_id = f'{date.strftime("%Y%m%d")}'
    task = SnowflakeOperator(
                    task_id=task_id,
                    snowflake_conn_id=snowflake_conn_id,
                    sql='rds/dunkin/ingest-dunkin-bos-adls-to-rds.sql',
                    warehouse=warehouse,
                    schema=schema,
                    dag=dag,
                    params={
    ...
    if date == end_date:
        task.set_downstream(trigger_audit_bos_crunchtime_sales)

trigger_audit_bos_crunchtime_sales.set_downstream(end_task)
