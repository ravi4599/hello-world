...
/*************************************calling metadata framework*********************************************/
    ...
    try {
      ...
      val stagePaymentDF =  spark.sql(spark.sparkContext.getConf.get("spark.source.sql").trim().replace("*whereclause*", whereClause))
      
      //+ whereClause
      log.info(s"""#---------------------------Query execution $stagePaymentDF ----------------#""")
      ...
      for (col <- required_columns) {
        ...
        if (hasColumn(stagePaymentDF, col)) {
      ...
      val stage_final_df = missing_columns.foldLeft(stagePaymentDF)((df, c) =>

        df.withColumn(s"$c", lit(null)))

       if(!stagePaymentDF.take(1).isEmpty){
