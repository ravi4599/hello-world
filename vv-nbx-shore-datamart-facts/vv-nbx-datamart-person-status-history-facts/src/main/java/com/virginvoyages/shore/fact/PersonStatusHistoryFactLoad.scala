...
object PersonStatusHistoryFactLoad {
...
/*************************************calling metadata framework*********************************************/
    ...
    try {
      ...
      val whereClause = s""" where ps.batchtime>= '$batch_start_tme' and ps.batchtime<='$batch_end_tme' and ps.part_date>='$part_read_start' and ps.part_date<='$part_read_end'"""
      ...
      val personstatusDF = spark.sql(spark.sparkContext.getConf.get("spark.source.sql").trim()+whereClause)
      //stageRevenueDF.show()
      ...
      for (col <- required_columns) {
        ...
        if (hasColumn(personstatusDF, col)) {
      ...
      val stage_final_df = missing_columns.foldLeft(personstatusDF)((df, c) =>
        df.withColumn(s"$c", lit("N/A")))

      println("xxxxxxxxxxxxxxxxxxxxxxxxxxxxx-------------------------------Source-Stage Folio Revenue Fact------------------------------------xxxxxxxxxxxxxxxxxxxxxx")
