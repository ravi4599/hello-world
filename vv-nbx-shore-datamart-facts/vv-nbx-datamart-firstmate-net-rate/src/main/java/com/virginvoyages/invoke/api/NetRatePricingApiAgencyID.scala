...
object NetRatePricingApiAgencyID {
  ...
  def main(args: Array[String]): Unit = {
    ...
    def parsePriceDetail(responseDF: DataFrame, configMap: Broadcast[Map[String, String]]): DataFrame = {
      ...
      var xmlStringRDD = dfnoEmpty.selectExpr("concat('<response> ',response,'<num_of_adults_passed> ',cast(guestnum as bigint),'<agency_id> ',cast(agency as bigint),'</agency_id> </num_of_adults_passed> </response>')").map(r => r.getString(0)).rdd
      xmlStringRDD.collect
      // xmlStringRDD.collect.foreach(println)

      var priceDetailDf = new XmlReader().xmlRdd(spark.sqlContext, xmlStringRDD)

      // priceDetailDf.show(20,false)
      priceDetailDf = priceDetailDf.withColumn("agency_id", priceDetailDf.col("agency_id"))
      ...
      return priceDetailDf.selectExpr("sail_date_from", "sail_date_to", "voyage_id", "currency", "amount", "price_code", "guestseq", "cast(num_of_adults_passed as int) as num_of_adults_passed", "cast(agency_id as int) as agency_id", "max_available_capacity", "PricedCategoryCode", "category", "capacity")
    }
    import spark.implicits._
    ...
    parseDF.select(col("sail_date_from"), col("sail_date_to"), col("voyage_id"), col("currency"), col("amount"), col("price_code"), col("guestseq"), col("num_of_adults_passed"), col("agency_id"), col("max_available_capacity"), col("PricedCategoryCode"), col("category"), col("capacity")).show(false)
    val parsefinalDF = parseDF.select(col("sail_date_from"), col("sail_date_to"), col("voyage_id"), col("currency"), col("amount"), col("price_code"), col("guestseq"), col("num_of_adults_passed"), col("agency_id"), col("max_available_capacity"), col("PricedCategoryCode"), col("category"), col("capacity"))

    //parsefinalDF.show()
    // parsefinalDF.write.mode("overwrite").parquet("s3://vv-prod-emr-cluster/tmp/backups/12Sep2022/test_netrate_pricing_api_test_new/")

    println("Intermediate data has been written to the location")

    parsefinalDF.createOrReplaceTempView("temp_price")

    val df1 = spark.sql("""SELECT sail_date_from,sail_date_to,voyage_id, agency_id, currency, amount, price_code, guestseq, num_of_adults_passed, max_available_capacity, pricedcategorycode, category,capacity from temp_price group by sail_date_from,sail_date_to,voyage_id,agency_id, currency, amount, price_code, guestseq, num_of_adults_passed, max_available_capacity, pricedcategorycode, category,capacity""")
    df1.createOrReplaceTempView("temp_price1")

    val df2 = spark.sql("""select p1.sail_date_from, p1.sail_date_to, p1.voyage_id, p1.agency_id, p1.currency, p1.amount, p2.amounts as Price_Total, p1.price_code, p1.price_codes, p1.guestseq, p1.num_of_adults_passed, p1.max_available_capacity, p1.PricedCategoryCode, p1.category, p1.capacity from ( select sail_date_from, sail_date_to, voyage_id, agency_id, currency, price_code, price_codes, amount, guestseq, num_of_adults_passed, max_available_capacity, PricedCategoryCode, category, capacity from ( select sail_date_from, sail_date_to, voyage_id, agency_id, currency, substr(price_code,1,3) as price_code, price_code as price_codes, amount, case when guestseq like '%1GT%' then 1 when guestseq like '%2GT%' then 2 when guestseq like '%3GT%' then 3 when guestseq like '%4GT%' then 4 else null end as guestseq , num_of_adults_passed, max_available_capacity, PricedCategoryCode, category, capacity from temp_price1) p0) p1 inner join ( select p0.amounts, p0.price_code, p0.voyage_id, p0.agency_id, p0.currency, p0.PricedCategoryCode, p0.num_of_adults_passed from ( select sum(amount) as amounts, substr(price_code,1,3) as price_code, voyage_id, agency_id, currency, PricedCategoryCode, num_of_adults_passed from temp_price1 where substr(price_code, 1, 4) in ('1GT/', '2GT/', '3GT/', '4GT/') and currency in ('GBP', 'AUD','CAD','USD','NZD') group by substr(price_code, 1, 3), voyage_id,agency_id, currency, PricedCategoryCode, num_of_adults_passed) p0) p2 on (p1.price_code = p2.price_code and p1.voyage_id = p2.voyage_id and p1.agency_id=p2.agency_id and p1.currency = p2.currency and p1.PricedCategoryCode = p2.PricedCategoryCode and p1.num_of_adults_passed = p2.num_of_adults_passed) order by price_codes""")
    val df3 = df2.select(col("sail_date_from"), col("sail_date_to"), col("voyage_id"), col("agency_id"), col("currency"), col("amount"), col("Price_Total"), col("price_code"), col("price_codes"), col("guestseq"), col("num_of_adults_passed"), col("max_available_capacity"), col("PricedCategoryCode"), col("category"), col("capacity")).withColumnRenamed("price_codes", "price_codes").withColumnRenamed("Price_Total", "price_totals")
    ...
    val sail_dim = spark.sql("""select pkg_dim.src_package_id as src_package_id, pkg_dim.package_code as package_code, NVL(sail_dim.sail_id, -1) as sail_id, sail_dim.src_sail_id as src_sail_id, sail_dim.sail_date_from as sail_date_from, sail_dim.sail_date_to as sail_date_to from ( select src_package_id, package_code, src_sail_id, upd_dt, rec_end_dttm, package_class from vv_db.hvtb_nbx_core_sw_package_dim where package_class = 'VOYAGE' and to_date(rec_end_dttm) = '9999-12-31') pkg_dim left join ( select sail_id, src_sail_id, sail_date_from, sail_date_to, upd_dt, is_active from vv_db.hvtb_nbx_core_sw_sail_dim where to_date(rec_end_dttm) = '9999-12-31') sail_dim on (pkg_dim.src_sail_id == sail_dim.src_sail_id)""")
    //val sail_dim = spark.sql("""select pkg_dim.src_package_id as src_package_id, pkg_dim.package_code as package_code, NVL(sail_dim.sail_id,-1) as sail_id, sail_dim.src_sail_id as src_sail_id, sail_dim.sail_date_from as sail_date_from, sail_dim.sail_date_to as sail_date_to from ( select src_package_id, package_code, src_sail_id, etl_ld_dt, rec_end_dttm, package_class from vv_db.hvtb_nbx_core_sw_package_dim where package_class = 'VOYAGE' and to_date(rec_end_dttm) = '9999-12-31') pkg_dim left join ( select sail_id, src_sail_id, sail_date_from, sail_date_to, etl_ld_dt, is_active from vv_db.hvtb_nbx_core_sw_sail_dim where to_date(rec_end_dttm) = '9999-12-31') sail_dim on (pkg_dim.src_sail_id == sail_dim.src_sail_id)""")
    sail_dim.show(false)
    sail_dim.createOrReplaceTempView("sail_dim")
    parseFilterDF.createOrReplaceTempView("pricing")

    val tempNewDF = spark.sql(s""" select price.voyage_id,price.agency_id,NVL(sail_dim.sail_id,-1) as sail_id,sail_dim.src_sail_id,price.price_code,price.price_codes,price.num_of_adults_passed ,price.max_available_capacity,price.PricedCategoryCode,price.guestseq,price.category,price.currency,price.amount,price.price_totals,sail_dim.sail_date_from,sail_dim.sail_date_to,price.load_dt  from pricing price left join sail_dim on(price.voyage_id == sail_dim.package_code)""")
    println("**** Printing Tranformed Data with joining condition ***********")
    tempNewDF.show()
    tempNewDF.createOrReplaceTempView("final_view")
    val tempDF = spark.sql("""select CAST(voyage_id as  string ) as voyage_id,CAST(agency_id as  string ) as agency_id,CAST(src_sail_id as  string ) as src_sail_id,CAST(sail_id as  long ) as sail_id,CAST(price_code as  string ) as price_code,CAST(price_codes as  string ) as price_codes,CAST(num_of_adults_passed as  string ) as num_of_adults_passed,CAST(max_available_capacity as  string ) as max_available_capacity,CAST(PricedCategoryCode as  string ) as PricedCategoryCode,CAST(guestseq as  long ) as guestseq,CAST(amount as  double ) as amount,CAST(price_totals as  double ) as price_totals,CAST(category as  string ) as category,CAST(currency as  string ) as currency,CAST(sail_date_from as  date ) as sail_date_from,CAST(sail_date_to as  date ) as sail_date_to,CAST(load_dt as  date ) as load_dt  from (select voyage_id,agency_id,src_sail_id,sail_id,price_code,price_codes,guestseq,num_of_adults_passed ,max_available_capacity,PricedCategoryCode,amount,category,currency,price_totals,sail_date_from,sail_date_to,load_dt,row_number() over(partition by src_sail_id,sail_id,price_code,price_codes,guestseq,amount,category,currency,price_totals order by load_dt)as rn from final_view)price""")
    ...
    spark.sql(""" insert overwrite table vv_db.hvtb_nbx_core_special_agency select * from pricetbl""")
