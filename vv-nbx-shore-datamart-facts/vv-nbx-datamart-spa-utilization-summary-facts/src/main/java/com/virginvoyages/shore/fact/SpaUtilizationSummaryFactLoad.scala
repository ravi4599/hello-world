...
object SpaUtilizationSummaryFactLoad  {
  // Creating logger
  ...
  def main(args: Array[String]): Unit = {
    ...
    try {
      ...
      val stageSpaUtilizationDF = spark.sql(query).as("sale_fact")
      println(query)
      log.info(query)
      
      if (!stageSpaUtilizationDF.head(1).isEmpty) {
        loadDimFact(spark, stageSpaUtilizationDF)
      ...
      //case e: SQLException => { ManageMetadata.updateStatus(batch_instance_id1, batch_id1, "Failed", spark); log.info("******************in the catch ofvv-nbx-datamart-SpaUtilizationSummaryFactLoad-factsn Load ******************"); e.printStackTrace(); throw new Exception("SQL Exception..please check the stacktrace", e); }

      case e: Exception =>
        { ManageMetadata.updateStatus(batch_instance_id1, batch_id1, "Failed", spark); log.info("******************in thevv-nbx-datamart-SpaUtilizationSummaryFactLoad-factsLoad ******************"); e.printStackTrace(); throw new Exception("General Exception..please check the stacktrace", e); }
