...
object MenuItemAllergyFactLoad {
...
/*************************************calling metadata framework*********************************************/
    ...
    try{
     ...
     //var MenuItemAllergyDf = spark.sql(spark.sparkContext.getConf.get("spark.source.sql").trim()+whereClause) 
     var MenuItemAllergyDf = spark.sql(spark.sparkContext.getConf.get("spark.source.sql").trim().replace("*whereClause*",whereClause))
     
     
    if(!MenuItemAllergyDf.head(1).isEmpty){

        log.info("Loading Menu Item Allergy table")
        loadDimFact(spark: SparkSession, MenuItemAllergyDf)
   ...
   ManageMetadata.updateStatus(batch_instance_id1, batch_id1, "Successful", spark)
       ...
       case e: SQLException => { 
         ...
         log.info("******************in the catch of Menu Item Allergy Load ******************");
         e.printStackTrace(); 
