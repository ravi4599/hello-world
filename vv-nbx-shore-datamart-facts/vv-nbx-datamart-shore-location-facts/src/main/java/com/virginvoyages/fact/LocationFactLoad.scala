...
/*************************************calling metadata framework*********************************************/
    ...
    try{
      ...
      val whereClause = s""" where locenter.batchtime>= '$batch_start_tme' and locenter.batchtime<='$batch_end_tme' and locenter.part_date >='$part_read_start' and locenter.part_date <='$part_read_end'"""
     ...
     if(!sourceDf.head(1).isEmpty){
       ...
       log.info("Loading Location fact table")
      ...
      case e: SQLException => { 
         ...
         log.info("******************in the catch of Location Fact Load ******************");
         e.printStackTrace(); 
