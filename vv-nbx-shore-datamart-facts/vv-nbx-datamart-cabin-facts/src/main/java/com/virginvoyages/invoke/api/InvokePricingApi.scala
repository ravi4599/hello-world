  ...
  def main(args: Array[String]): Unit = {
    ...
    def parsePriceDetail(responseDF: DataFrame, configMap: Broadcast[Map[String, String]]): DataFrame = {
      ...
      if (checkArray(priceDetailDf, "AvailCategories_new")) {
        ...
        priceDetailDf = priceDetailDf.withColumn("promo_code", priceDetailDf.col("AvailCategory_explode.UsedPromos.PromoItem.PromoCode"))
      ...
      return priceDetailDf
        .selectExpr("src_sail_id", "currency", "price_totals", "price_code", "amount", "guestseq", "cast(num_of_adults_passed as int) as num_of_adults_passed", "max_available_capacity", "capacity", "category","promo_code")
    ...
    val tempNewDF = spark.sql(s""" select price.src_sail_id,NVL(dim.sail_id,-1) as sail_id,cast(price.price_code as string) as price_code,price.num_of_adults_passed ,price.max_available_capacity,price.guestseq ,price.amount ,price.category,price.currency,price.price_totals ,price.promo_code,dim.sail_date_from,dim.sail_date_to,price.load_dt  from pricing price left join vv_db.hvtb_nbx_core_sw_sail_dim dim on(price.src_sail_id == dim.src_sail_id)where to_date(rec_end_dttm) = '9999-12-31' """)
    val tempOldDF = spark.sql("select * from " + pricingTable + " where sail_date_from<current_date()")
    
    
    println(tempNewDF.schema)
    println(tempNewDF.show(100))
    println("-----------------------------------------------------")
    println(tempOldDF.show(100))
    println(tempOldDF.schema)
    ...
    val tempDF = spark.sql("select CAST(src_sail_id as  string ) as src_sail_id,CAST(sail_id as  long ) as sail_id,CAST(price_code as  string ) as price_code,CAST(num_of_adults_passed as  string ) as num_of_adults_passed,CAST(max_available_capacity as  string ) as max_available_capacity,CAST(guestseq as  long ) as guestseq,CAST(amount as  double ) as amount,CAST(category as  string ) as category,CAST(currency as  string ) as currency,CAST(price_totals as  long ) as price_totals,CAST(promo_code as  string ) as promo_code,CAST(sail_date_from as  date ) as sail_date_from,CAST(sail_date_to as  date ) as sail_date_to,CAST(load_dt as  date ) as load_dt  from (select src_sail_id,sail_id,price_code,guestseq,num_of_adults_passed ,max_available_capacity,amount,category,currency,price_totals,promo_code,sail_date_from,sail_date_to,load_dt,row_number() over(partition by src_sail_id,sail_id,price_code,guestseq,amount,category,currency,price_totals order by load_dt)as rn from final_view)price")
  
    
    log.info(tempDF)
    spark.sql("refresh table " + pricingTable)
    tempDF.write.mode("Overwrite").parquet(spark.sparkContext.getConf.get("spark.target.temp").trim())
    spark.sql("Alter table " + pricingTable + " set location '" + spark.sparkContext.getConf.get("spark.target.temp").trim() + "'")
    spark.sql("refresh table " + pricingTable)

    val finalDF = spark.read.parquet(spark.sparkContext.getConf.get("spark.target.temp").trim()).dropDuplicates()
    //finalDF.printSchema()
    finalDF.write.mode("Overwrite").parquet(spark.sparkContext.getConf.get("spark.target.final").trim())
    //sys.exit(1)
    spark.sql("Alter table " + pricingTable + " set location '" + spark.sparkContext.getConf.get("spark.target.final").trim() + "'")
    spark.sql("refresh table " + pricingTable)
    //.write.mode("Overwrite").insertInto(pricingTable)
