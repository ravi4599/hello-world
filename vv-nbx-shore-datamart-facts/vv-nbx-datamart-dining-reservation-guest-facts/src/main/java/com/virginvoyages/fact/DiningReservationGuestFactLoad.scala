...
object DiningReservationGuestFactLoad {
  // Creating logger
  ...
  def main(args: Array[String]): Unit = {
    ...
    try {
      ...
      val DinReserveDF = spark.sql(query).as("dining_reservation_guest_fact")
      println(query)
      log.info(query)
      
     
   
      if (!DinReserveDF.head(1).isEmpty) {
        loadDimFact(spark, DinReserveDF)
