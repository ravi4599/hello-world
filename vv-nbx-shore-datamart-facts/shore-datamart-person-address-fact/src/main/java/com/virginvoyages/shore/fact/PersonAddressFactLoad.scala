...
object PersonAddressFactLoad {
  ...
  def main(args: Array[String]): Unit = {
    ...
    try {
      ...
      val stageLovelistDF = spark.sql(query).as("lovelist_fact")
      println(query)
      log.info(query)
      
      if (!stageLovelistDF.head(1).isEmpty) {
        loadDimFact(spark, stageLovelistDF)
      ...
      //case e: SQLException => { ManageMetadata.updateStatus(batch_instance_id1, batch_id1, "Failed", spark); log.info("******************in the catch ofvv-nbx-datamart-PersonAddressFactLoad-factsn Load ******************"); e.printStackTrace(); throw new Exception("SQL Exception..please check the stacktrace", e); }

      case e: Exception =>
        { ManageMetadata.updateStatus(batch_instance_id1, batch_id1, "Failed", spark); log.info("******************in thevv-nbx-datamart-lovelist-factsLoad ******************"); e.printStackTrace(); throw new Exception("General Exception..please check the stacktrace", e); }
