package com.virginvoyages.revenuecommissionfact
  ...
  def main(args: Array[String]) {
    ...
    try {
      ...
      if (args.length == 0) {
        ...
        val temp_qry = s"call seaware.revenue_commission_fact_rpt_sp()"
        try {
      ...
      case e: Exception => {
        log.info("******************in the catch of forecastactulas ******************");
        e.printStackTrace();
