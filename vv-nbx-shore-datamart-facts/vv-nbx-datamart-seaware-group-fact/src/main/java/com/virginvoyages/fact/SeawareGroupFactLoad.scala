...
/*************************************calling metadata framework*********************************************/
    ...
    try {
     ...
     val whereClause = s""" where batchtime>= '$batch_start_tme' and batchtime<='$batch_end_tme' and part_date>='$part_read_start' and part_date<='$part_read_end'"""
      ...
      val GroupFactDF =  spark.sql(spark.sparkContext.getConf.get("spark.source.sql").trim().replace("*whereclause*", whereClause))
      /*.replace("*whereclause*", whereClause)*/
          /*.trim()+whereClause)*/
     /* val stageVoyageDF = spark.sql(spark.sparkContext.getConf.get("spark.source.sql"))*/
      //GroupFactDF.show(4,false)


     val interDF=GroupFactDF.select($"SNAPSHOT_time",$"GROUP_ID",$"GUEST_SEQN",$"PACKAGE_ID",$"INVOICE_ITEM_TYPE_ID",$"PRICE_AREA_ID",$"SHIP_ID",$"AGENT_ID",$"AGENCY_ID",$"PROMOTION_ID",$"ADDON_ID",$"CARDECK_ID",$"SHOREX_ID",$"HOTEL_GROUP_REQ_ID",$"SAIL_ID",$"AMOUNT",$"COMMISSION_PERCENT",$"QUANTITY",$"CURRENCY",$"CURRENCY_RATE",$"CABIN_CATEGORY",$"GENERIC_CATEGORY",$"SNAPSHOT_DATE")
      ...
      println("xxxxxxxxxxxxxxxxxxxxxxxxxxxxx-------------------------------Source-Group------------------------------------xxxxxxxxxxxxxxxxxxxxxx")
     ...
     if (!stage_final_df.head(1).isEmpty) {
      ...
      case e: SQLException => { ManageMetadata.updateStatus(batch_instance_id1, batch_id1, "Failed", spark); log.info("******************in the catch of Group Fact Load ******************"); e.printStackTrace(); throw new Exception("SQL Exception..please check the stacktrace", e); }

      case e: Exception =>
       { ManageMetadata.updateStatus(batch_instance_id1, batch_id1, "Failed", spark); log.info("******************in the catch of Group Fact Load ******************"); e.printStackTrace(); throw new Exception("General Exception..please check the stacktrace", e); }
