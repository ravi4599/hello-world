...
object PositionOrgUnitDetailFact {
   // Creating logger
   ...
   val partReadEnd = metadata._6
    ...
    try {
     ...
     import spark.sqlContext.implicits._
      ...
      println(query)
      log.info(query)      
      val finalcolDf = spark.sql(query).as("position_fact")
      finalcolDf.show(false)

    if (!finalcolDf.head(1).isEmpty) {
        loadDimFact(spark, finalcolDf)
      ...
      case e: Exception =>
        {
          ...
          log.info("******************in the vv-nbx-datamart-shore-position-orgunit-detail-facts ******************");
          e.printStackTrace();
