...
/*************************************calling metadata framework*********************************************/
    ...
    try{
     ...
     println(query)
      ...
      var ChatQueueDf = spark.sql(query)
     
     if(!ChatQueueDf.head(1).isEmpty){
     
        log.info("Loading Dim table")
       loadDimFact(spark: SparkSession, ChatQueueDf)
      ...
      ManageMetadata.updateStatus(batch_instance_id1, batch_id1, "Successful", spark)
       ...
       case e: SQLException => { 
         ...
         log.info("******************in the catch of Fact Load ******************");
         e.printStackTrace(); 
