	...
	val log = LogManager.getRootLogger
		...
		val spark = getSparkSession()
				...
				try{
					...
					println(s"Batch_start_time is $batch_start_tme and  batch_end_time is $batch_end_tme ")
					...
					//var diningResDf = spark.sql(spark.sparkContext.getConf.get("spark.source.sql").trim()+whereClause)
					val diningResDf = spark.sql(spark.sparkContext.getConf.get("spark.source.sql").replace("*whereclause*", whereClause))
					diningResDf.show(5,false)


					if(!diningResDf.head(1).isEmpty){

						log.info("Loading Dining Res Item fact table")
						loadDimFact(spark: SparkSession, diningResDf)
		...
		case e: SQLException => { 
			...
			log.info("******************in the catch of Dining Reservation Item Fact Load ******************");
			e.printStackTrace(); 
