	...
	val log = LogManager.getRootLogger
		...
		val spark = getSparkSession()
				...
				try{
					...
					val whereClause = s""" where batchtime>= '$batch_start_tme' and batchtime<='$batch_end_tme' and part_dt >='$part_read_start' and part_dt <='$part_read_end'"""

							println(s"========================== $whereClause =========================")
							log.info (s"===================== $whereClause =============================")

							

							var allergyDf = spark.sql(spark.sparkContext.getConf.get("spark.source.sql").trim().replace("*whereclause*", whereClause))
							
							allergyDf.printSchema()
							allergyDf.show(5,false)
							

							if(!allergyDf.head(1).isEmpty){

								log.info("Loading Allergy fact table")
								loadDimFact(spark: SparkSession, allergyDf)
		...
		case e: SQLException => { 
			...
			log.info("******************in the catch of Allergy Fact Load ******************");
			e.printStackTrace(); 
