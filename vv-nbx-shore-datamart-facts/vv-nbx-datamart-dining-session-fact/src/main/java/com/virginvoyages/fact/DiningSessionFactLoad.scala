...
try{
//sourceDf.show()
val whereClause = s""" where sessions.batchtime>= '$batch_start_tme' and sessions.batchtime<='$batch_end_tme' and sessions.part_date >='$part_read_start' and sessions.part_date <='$part_read_end'"""
...
log.info("Loading Dining Sesssion fact table")
...
log.info("******************in the catch of Dining Session Fact Load ******************");
e.printStackTrace();
