package com.virginvoyages.vxp.fact
...
object HousekeepingRequestFact {
...
/*************************************calling metadata framework*********************************************/
    ...
    try{
      ...
      val whereClause = s""" where requests.batchtime>= '$batch_start_tme' and requests.batchtime<='$batch_end_tme' and requests.part_date >='$part_read_start' and requests.part_date <='$part_read_end'"""
     ...
     if(!sourceDf.head(1).isEmpty){
      ...
      case e: SQLException => { 
         ...
         log.info("******************in the catch of Request Fact Load ******************");
         e.printStackTrace(); 
