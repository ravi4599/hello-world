...
//      val whereClause = s""" where 1=1 """
      ...
      val stagePersonStatusDF = spark.sql(query).as("sale_fact")
      println(query)
      log.info(query)
      
      stagePersonStatusDF.show(5,false)
      
      if (!stagePersonStatusDF.head(1).isEmpty) {
        loadDimFact(spark, stagePersonStatusDF)
      ...
      case e: Exception =>
        { ManageMetadata.updateStatus(batch_instance_id1, batch_id1, "Failed", spark); log.info("******************in thevv-nbx-datamart-person-status-factsLoad ******************"); e.printStackTrace(); throw new Exception("General Exception..please check the stacktrace", e); }
