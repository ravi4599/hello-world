...
object EnablerFrameworkWithKey {
    ...
    */
    ...
    messages.foreachRDD { (rdd, batchTime) =>
       ...
       */
        ...
        //var keyvalDf = rdd.map(row => row.key()).toDF("Key") 
        
      //var messageDf = rdd.map(row => row.value()).toDF("Message")  
      
      var keymessagevalDf=rdd.map(row=>(row.key(),row.value())).toDF("keyattr","Message")
        
        if (!keymessagevalDf.head(1).isEmpty) {
          /*val keyArr = keyvalDf.selectExpr("Key").rdd.map(x=>x.mkString).collect
          val key = keyArr(0)*/
          
       /* val key = keyvalDf.select("Key").head(1)(0).mkString
          log.info(" Key::::" + key)
          keyvalDf.printSchema()  */
        
          /*-
             * Writing the messsage to landing table as it is along with audit columns
          */
        /*val finalDf = messageDf.withColumn("batchtime", lit(startExecutionTime).cast(TimestampType)).withColumn("batchinstanceid", lit(batchInstanceId)).withColumn("part_date", to_date(lit(partWriteDate))).withColumn("voyageid", lit(voyageId).cast(StringType))
          .withColumn("shipcode", lit(shipCode).cast(StringType)).withColumn("keyattr", lit(key).cast(StringType))
            .select("batchinstanceid", "voyageid", "batchtime", "message","shipcode", "keyattr","part_date")  */
          
          
          
            val finalDf =keymessagevalDf.withColumn("batchtime", lit(startExecutionTime).cast(TimestampType)).withColumn("batchinstanceid", lit(batchInstanceId)).withColumn("part_date", to_date(lit(partWriteDate))).withColumn("voyageid", lit(voyageId).cast(StringType))
          .withColumn("shipcode", lit(shipCode).cast(StringType)).select("batchinstanceid", "voyageid", "batchtime", "message","shipcode", "keyattr","part_date")
