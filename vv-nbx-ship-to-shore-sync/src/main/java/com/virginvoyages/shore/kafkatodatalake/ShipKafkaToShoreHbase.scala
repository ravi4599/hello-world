			...
			def main(args: Array[String]): Unit = {
					val topics = Array(spark.sparkContext.getConf.get("spark.src.kafka.topic").trim)	
							/*val kafkaParams = Map[String, Object](
									ConsumerConfig.BOOTSTRAP_SERVERS_CONFIG -> spark.sparkContext.getConf.get("spark.src.brokers").trim(),
...
*/
			...
			//capturing current data and time used across in metadata entries 
        val currentTime  =DateTimeFormatter.ofPattern("yyyy-MM-dd_HH:mm").format(LocalDateTime.now)
    var sslflag = true
    ...
    if (sslflag == false) {
      kafkaParams = Map[String, Object](
        ...
        ConsumerConfig.VALUE_DESERIALIZER_CLASS_CONFIG -> classOf[StringDeserializer],
        ConsumerConfig.GROUP_ID_CONFIG -> spark.sparkContext.getConf.get("spark.src.consumer").trim(),
        ConsumerConfig.MAX_PARTITION_FETCH_BYTES_CONFIG -> spark.sparkContext.getConf.get("spark.streaming.kafka.maxRatePerPartition").trim(),
       // ConsumerConfig.AUTO_OFFSET_RESET_CONFIG -> spark.sparkContext.getConf.get("spark.streaming.kafka.maxRatePerPartition").trim(),
        ConsumerConfig.ENABLE_AUTO_COMMIT_CONFIG -> (false: java.lang.Boolean))
    } else  if (sslflag == true) {
      kafkaParams = Map[String, Object](
        ConsumerConfig.BOOTSTRAP_SERVERS_CONFIG -> spark.sparkContext.getConf.get("spark.shore.sslkafkabrokers").trim,
        ConsumerConfig.KEY_DESERIALIZER_CLASS_CONFIG -> classOf[StringDeserializer],
        ConsumerConfig.VALUE_DESERIALIZER_CLASS_CONFIG -> classOf[StringDeserializer],
        ConsumerConfig.GROUP_ID_CONFIG -> spark.sparkContext.getConf.get("spark.arsbooking.consumer").trim(),
        //ConsumerConfig.AUTO_OFFSET_RESET_CONFIG -> spark.sparkContext.getConf.get("spark.arsbooking.reservationOffSet").trim(),
        ConsumerConfig.MAX_PARTITION_FETCH_BYTES_CONFIG -> spark.sparkContext.getConf.get("spark.arsbooking.partition.byte").trim(),
        ConsumerConfig.ENABLE_AUTO_COMMIT_CONFIG -> (false: java.lang.Boolean))
									...
									try 
									{
										var msg_df=spark.read.json(msgTuple)
												...
												if(!msg_df.head(1).isEmpty){
													val VOYAGEID=spark.sparkContext.getConf.get("spark.target.table") //Changed to spark.target.table
													...
													var final_df=json_df.select(column_seq1:_*)
															...
															//addMetadataShipShr(VOYAGEOPS_df1,VOYAGEID,SRCCONFIGID,TGTCONFIGID,spark) //add shipshore metadata entry
