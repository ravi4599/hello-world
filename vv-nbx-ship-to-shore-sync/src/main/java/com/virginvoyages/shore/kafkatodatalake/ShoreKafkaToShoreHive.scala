...
object ShoreKafkaToShoreHive  {
			...
			def main(args: Array[String]): Unit = {
					val topics = Array(spark.sparkContext.getConf.get("spark.src.kafka.topic").trim)	
							...
							/*CommonClientConfigs.SECURITY_PROTOCOL_CONFIG -> "SSL",
									SslConfigs.SSL_TRUSTSTORE_PASSWORD_CONFIG -> spark.sparkContext.getConf.get("spark.src.passvalue").trim,
									SslConfigs.SSL_TRUSTSTORE_LOCATION_CONFIG -> spark.sparkContext.getConf.get("spark.src.truststore").trim,
									SslConfigs.SSL_KEYSTORE_LOCATION_CONFIG -> spark.sparkContext.getConf.get("spark.src.keystore").trim,
									SslConfigs.SSL_TRUSTSTORE_LOCATION_CONFIG -> spark.sparkContext.getConf.get("spark.src.truststore").trim,
									SslConfigs.SSL_KEYSTORE_LOCATION_CONFIG -> spark.sparkContext.getConf.get("spark.src.keystore").trim,
									SslConfigs.SSL_KEYSTORE_PASSWORD_CONFIG -> spark.sparkContext.getConf.get("spark.src.passvalue").trim,
									SslConfigs.SSL_KEY_PASSWORD_CONFIG -> spark.sparkContext.getConf.get("spark.src.passvalue").trim)*/
							...
							var transCommit: Boolean = true; 
									...
									log.info("count of rddd")
									log.info(msgTuple.count())
									try {
										var hive_df=spark.read.json(msgTuple)
												//	hive_df.show()
												if(!hive_df.head(1).isEmpty){
													var VOYAGEOPS_parameter=spark.sparkContext.getConf.get("spark.target.VOYAGEOPS").split(",")
															var VOYAGEID=VOYAGEOPS_parameter(0)
															var SRCCONFIGID=VOYAGEOPS_parameter(1)
															var TGTCONFIGID=VOYAGEOPS_parameter(2)
															var TYPE=VOYAGEOPS_parameter(3)
															val df= Seq(VOYAGEID)
															...
															//var column_names1="C360ID,MASTER_ID,CRM_ID,SEAWARE_ID,VXP_ID,TIMESTAMP,SAILOR_TRIBE,SAILOR_SUBTRIBE,ISACTIVE"//spark.sparkContext.getConf.get("spark.colname").trim
															var column_names1=spark.sparkContext.getConf.get("spark.target.table.schema").trim
															/*var column_array1=column_names1.split(",")
															var column_seq1=column_array1.map(x=>col(x)).toSeq
															json_df.select(column_seq1:_*).show()
													...
													var final_df=json_df.select(column_seq1:_*)
															//	final_df.show(5)
															final_df.createOrReplaceTempView("temptable")
															var insert_hive_qry=spark.sparkContext.getConf.get("spark.insert.hive.qry").trim
															spark.sql("%s select * from temptable".format(insert_hive_qry))
															//spark.sql("insert overwrite table vv_test.HIVE_TRIBE_SUBTRIBE2 select * from temptable")
															//final_df.coalesce(spark.sparkContext.getConf.get("spark.dataframe.parttition").toInt).write.mode(spark.sparkContext.getConf.get("spark.write.mode").trim).parquet(spark.sparkContext.getConf.get("spark.write.path").trim)
															...
															//VOYAGEOPS_df=VOYAGEOPS_df.withColumn("STATUS",lit("succeeded").cast(StringType)).withColumn("ENDTIME",lit(current_timestamp()))
									...
									var VOYAGEOPS_parameter=spark.sparkContext.getConf.get("spark.target.VOYAGEOPS").split(",")
											var VOYAGEID=VOYAGEOPS_parameter(0)//"VOYAGEID12"
											var SRCCONFIGID=VOYAGEOPS_parameter(1)//"SRCCONFIGID11"
											var TGTCONFIGID=VOYAGEOPS_parameter(2)//"TGTCONFIGID11"
											var TYPE=VOYAGEOPS_parameter(3)//"sync"
											val df= Seq(VOYAGEID)
											import java.time.format.DateTimeFormatter
											import java.time.LocalDateTime 
											val voyageOperRowKey = VOYAGEID.concat("-").concat(SRCCONFIGID).concat("-").concat(TGTCONFIGID)
	...
	def createXMLMsgs(msgRdd: RDD[ConsumerRecord[String, String]]) = {
			log.info("insidee create xml msgggggggggggggggggg")
