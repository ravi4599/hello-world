...
/**********************************MetaData framework**********************************************/
    ...
    try {
      ...
      var cticampaigndf = Seq(jsonMessage).toDF("Message")
      
      cticampaigndf = cticampaigndf.withColumn("batchtime", unix_timestamp(lit(batchStartTime), "yyyy-MM-dd' 'HH:mm:ss").cast(TimestampType))
        .withColumn("part_date", lit(to_date(col("batchtime"), "yyyy-MM-dd")))

      cticampaigndf = cticampaigndf.select("batchtime", "Message", "part_date")
      cticampaigndf.show(10, false)
      //cticampaigndf.printSchema()

      cticampaigndf.write.mode("overwrite").insertInto(spark.sparkContext.getConf.get("spark.source.cticampain.table").trim())
  ...
  def geJsonMessage(access_token: String): String = {
    ...
    //val url = "https://api-c29.incontact.com/inContactAPI/services/v17.0/campaigns"
    val url = "https://api-na1.niceincontact.com/inContactAPI/services/v18.0/campaigns"
    val log = LogManager.getRootLogger
    log.setLevel(Level.INFO)
