...
/**********************************MetaData framework**********************************************/
    ...
    try {
      ...
      //ctiagentdf.show(10, false)
      //ctiagentdf.printSchema()

      ctiagentdf.write.mode("overwrite").insertInto(spark.sparkContext.getConf.get("spark.source.ctiagent.table").trim())
  ...
  def geJsonMessage(access_token: String): String = {
	...
	val url = "https://api-na1.niceincontact.com/inContactAPI/services/v28.0/agents"
    val log = LogManager.getRootLogger
    log.setLevel(Level.INFO)
