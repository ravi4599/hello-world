...
/**********************************MetaData framework**********************************************/
    ...
    try {
      ...
      var ctiteamdf = Seq(jsonMessage).toDF("Message")
      
      ctiteamdf = ctiteamdf.withColumn("batchtime", unix_timestamp(lit(batchStartTime), "yyyy-MM-dd' 'HH:mm:ss").cast(TimestampType))
        .withColumn("part_date", lit(to_date(col("batchtime"), "yyyy-MM-dd")))

      ctiteamdf = ctiteamdf.select("batchtime", "Message", "part_date")
     // ctiteamdf.show(10, false)
     // ctiteamdf.printSchema()

      //ctiteamdf.write.mode("overwrite").insertInto(spark.sparkContext.getConf.get("spark.source.ctiteam.table").trim())
	  ctiteamdf.write.mode("overwrite").partitionBy("part_date").parquet(spark.sparkContext.getConf.get("spark.source.ctiteam.path").trim())
 ...
 def getAccessToken(configMap: Broadcast[Map[String, String]],spark:SparkSession): String = {
  ...
  def geJsonMessage(access_token: String): String = {
    val url = "https://api-na1.niceincontact.com/inContactAPI/services/v18.0/teams"
    //val url = "https://api-c29.incontact.com/inContactAPI/services/v17.0/teams"
    val log = LogManager.getRootLogger
    log.setLevel(Level.INFO)
